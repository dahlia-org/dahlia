...
## 正準モデル(新規 `apps/desktop/Sources/Dahlia/Models/SummaryDocument.swift`)
...
## LLM 出力(新規 `apps/desktop/Sources/Dahlia/Models/SummaryDocumentResponse.swift`)
...
## プロンプト変更(`apps/desktop/Sources/Dahlia/Models/AppSettings.swift` L382 付近)
...
## レンダリング層(新規 `apps/desktop/Sources/Dahlia/Services/SummaryRendering/`)
...
1. **AST モデル**: `Models/SummaryDocument.swift` + 新規 `apps/desktop/Tests/DahliaTests/SummaryDocumentCodableTests.swift`(Codable round-trip、未知 type → paragraph フォールバック、Swift Testing で記述)
...
7. **清掃**: 旧 API・旧テスト削除、`apps/desktop/Sources/Dahlia/CLAUDE.md` の LLM 行(「`SummaryResult` 構造化出力」記述)を `SummaryDocument` に更新

## 検証

- `swift test`(**注意: この環境では exit 0 のままテスト未実行になることがある。`Test run with N tests` の集計行が出力されているか必ず確認** — `apps/desktop/Tests/DahliaTests/CLAUDE.md` 記載)、`swift test --filter <各新規スイート>`

...
失敗時には v41 の状態にロールバックして再試行できる。次回公開後の変更は後続の forward migration として追加する。
...
`v42_localFirstSchema` は meeting に同期用カレンダー working copy の nullable 列を追加する。端末固有の予定参照と公開済みデータは変更せず、Server からの値（明示的な null を含む）を保持して古いローカル予定による再送上書きを防ぐ。

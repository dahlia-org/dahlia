...
[文字起こし・要約の処理場所](../shared/transcription-summary-processing.md)に従い、Server Accountは `processing.location` でlocal／remoteを共有する。アカウントの `summary.style`・出力言語と、`processing.remote` のworkflow・モデル・推論強度を分離する。workflowは文字起こし後に要約／一括生成を明示し、省略モデル・推論強度は受付時に自動解決する。処理場所・workflowの切替で既存overrideを削除しない。Mac推論設定はどのアカウントにも紐付けない。

local選択中はDesktopのApple Speechと内蔵Codexを使い、Server要約APIを呼ばない。remoteは常に録音入力を使う。開始済みジョブの固定設定とDesktopの永続スナップショットは維持し、失敗時の自動切替は行わない。以下の2026-09-08節にある方式別設定の記述は、この統合判断に置き換えられた。
...
上限超過は切り捨てた要約を保存せず失敗とする。画像本体は[スクリーンショットの選別](#スクリーンショットの選別2026-09-26)後に時間順で最大24枚をサンプルし、画像の OCR / caption / 選別理由と未送信画像の metadata はモデル入力に含めない。
...
会議・Project と既存方式で選別・サンプルする最大24枚の画像を併用する。文字起こし本文は収集・送信しない。
...
## スクリーンショットの選別（2026-09-26）

自動撮影は30秒ごとに画面変化20%以上で撮るため、同じ画面や顔・カメラ映像が24枚の枠を占め、共有資料が漏れていた。
Server の画像解析を同じ会議・同じ依頼者の最大 `DAHLIA_IMAGE_ANALYSIS_BATCH_SIZE` 枚（既定12、1〜24）の単位で行い、
OCR / caption に加えて有用性、1文の理由、直前画像と同じ内容かを同じ応答で返させる。バッチ先頭の直前の撮影画像を参照画像として添え、
バッチ境界をまたぐ重複も判定する。細かい分類 enum は持たない。失敗したバッチは各画像を単独で再試行する。

判定は file metadata の同期契約に入れず、Server 専用で再作成可能な `app.screenshot_assessments` に保存する（FORCE RLS）。
暗号化 Workspace では画像内容を表す理由を保存しない。既存の OCR / caption がある画像も判定がなければ `fill_missing` で解析し、
file revision は変えない。要約は有用でない画像、会議に残る直前画像と同じ画像、同一 hash の画像を除き、残りを従来どおり等間隔に選ぶ。
未判定の画像は候補に残す。隣接しない再表示は hash が同じ場合だけ除外する。
...
内部 `revision` は実際に値が変わったときだけ増やし、SSEの変更検知専用とする。

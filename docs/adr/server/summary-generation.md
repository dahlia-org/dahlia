...
[文字起こし・要約の処理場所](../shared/transcription-summary-processing.md)に従い、Server Accountは `summary.mode` でlocal／remoteを共有する。remote設定は `detail`、要約`model`、`reasoningEffort`、任意の`transcriptionModel`に統合する。`transcriptionModel`ありは文字起こし後に要約、なしは録音から文字起こしと要約を一括生成する。旧`method`、`methodSettings`、`cloudTranscription`の公開契約は廃止する。
...
機能検出は `GET /api/v1/capabilities` の `meetingSummaryGeneration: { version, sources }` に統合する。統合設定契約はversion 2とし、登録済み方式から sources を導出する。未対応はキーを省略し、capabilities 自体が空の場合も未対応とする。要約設定は `summary.mode` と `summary.remote` にまとめ、PATCH は指定した葉だけ更新し、`transcriptionModel: null`だけはその葉を削除する。開始済みジョブの設定スナップショットは変更しない。`outputLanguage` はアカウント設定直下に維持する。

...
個人所有 Vault を owner が特定 Organization / Team / User へ明示的に read-only 共有する。write、delete、共有設定変更は owner のみ。共有機能は常に有効とし、機能を無効化する環境設定は設けない。
...
- Private Web はボタンから開くモーダルで共有先を検索する。候補は owner の所属組織、そのチーム、同じ組織のユーザーに限定する。User への直接共有は付与時に同じ組織への所属を確認し、その後は owner が取り消すまでユーザー本人に閲覧権限を保持する。既存の共有先は所属変更後も owner が検索・解除できる。

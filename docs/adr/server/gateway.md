...
モデル一覧の正本を backend へ移し、`/admin/models` と管理 API を廃止する。2026-09-06 の未リリース baseline 整理で、不要な Model Alias テーブル・CRUD・公開型と Gateway constructor の store 引数も削除した。Databricks は必須の `DATABRICKS_MODEL_SCHEMA` 配下を App service principal で取得し、`supported_api_types` に `mlflow/v1/responses` を含むモデルだけを短い名前で公開する。対応情報の欠落・空配列は除外し、不正型は一覧取得エラーとする。通常の Responses は backend 内でスキーマを補完し、利用者の OBO token で転送する。認証済み userId は Databricks request tags の user_id に設定する。OpenAI と Cloudflare の一覧は当面 gpt-5.6-luna の mock とする。

...
`search_documents` を meeting / Project / screenshot の共通 registry とし、contentless FTS と vector を再構築可能な projection にする。source table trigger は coalesced job を積むだけで、utility worker が解析・索引する。録音中は FTS、画像解析、vector worker を停止する。検索失敗は確定データ保存を待たせない。
...
Project は専用結果と明示 filter で探す。Project name / path を meeting の Simple / Advanced 検索から除外し、Project vector による meeting 順位補正もしない。要約生成に既に使った Project 文脈を独立の証拠として二重評価しないためである。Project 専用文書・FTS column と既存 vector は保持できる。
...
重みは UserDefaults の policy を呼出引数で渡し、開発者設定に preset / slider を置く。preset は重みから導出する。MCP は既定重みだけを使い app 設定を読まないため、app と順位が異なり得る。Simple は比較用 metadata LIKE で、重みと summary 本文を反映しない。一文字・語中一致は Simple に限る。
...
screenshot は独立結果と `query_screenshots` で返し、meeting に集約せず vector を作らない。caption は解釈なので BM25 重み0.4で OCR より下げる。要約には抽出 text ではなく画像を渡し続ける。アプリ言語を認識候補、要約言語を caption に使い、変更は将来解析分へだけ反映する。画像解析の Codex 経路はローカル vector 推論とは別であり、全画像分の AI 処理量と DB 容量が増える。

...
- metadata は application database、bytes は設定可能な local、S3互換、R2、または Databricks managed Volume に保存する。artifact は作成者の personal workspace が所有し、既定は private、明示的な `PATCH` だけで public にする。削除済み UUID は reservation tombstone で永久に再利用を拒否し、既存 public URL の乗っ取りを防ぐ。指定ユーザー共有、一覧、履歴 API は持たない。
...
- storage backend は認可後に Range を含む response を Dahlia API 経由で streaming relayする。relay には CSP sandbox を付け、任意 HTML に Dahlia application origin の権限を与えない。storage URLやcredentialはclientへ返さない。
...
- storage credential は client に配布しない。R2 binding、S3 credential、Databricks App service principal の `WRITE_VOLUME` は deployment が管理する。

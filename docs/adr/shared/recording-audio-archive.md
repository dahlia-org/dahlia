...
- 日付: 2026-09-07
- 状態: 採用（2026-09-16 の Local／Server 境界で一部上書き）
...
元 CAF は、全音源の確定保存・復号検証・文字起こし成功を満たした後（Server は再取得照合も必須）に既存 purge state machine で解放する。
CAF の削除失敗は保存済み M4A の利用可能状態を変更せず、削除開始済みなら通信・保存検証を繰り返さず既存の削除処理を再試行する。再文字起こしは削除済み・削除保留の CAF を入力にせず、保存済みアーカイブを使用する。
...
新規 Local Workspace のバッチ録音は `recording_archives` を作らず、音源別の検証済み CAF を設定された
保持期間だけ保存する。M4A への結合保存と CAF の早期削除は行わない。保持期間内の再文字起こしは CAF を入力とし、
区間ごとに言語を自動判定して Apple Speech で処理する。既存 Local M4A と準備途中の archive は互換入力として読み取り、
再変換や期限前の削除は行わず、期限到達時に CAF と同じ既存 purge 処理で削除する。

Server Account 配下の Workspace は従来どおり全バッチ録音を音源別 M4A に変換してアップロードし、確定・再取得照合後に
CAF を削除する。初回文字起こしの処理場所、workflow、言語設定は維持する。明示的な再文字起こしはアップロード済みの
完全な M4A 集合だけを入力とする Server Gemini 専用処理とし、言語指定を送らず、Apple Speech や端末処理へ
フォールバックしない。アップロードの未完了・失敗時は初回結果を保持し、再文字起こしだけを利用不可にする。

保持期間は Local Workspace の CAF と互換 M4A にだけ適用し、Server の確定 M4A には適用しない。変換、アップロード、
再文字起こしが未完了・失敗・再試行中の音声は既存の状態機械で保護する。会議／Workspace の明示削除は Local CAF、
互換 M4A、Server staging／確定 M4A と処理ジョブを既存の cascade と削除キューで除去し、遅延結果を復活させない。
...
新しい sync entity を旧 Desktop が誤読しないよう、Server は `sync: { version: 3 }` と
`recordingArchive: { version: 1 }` を広告する。Desktop は常に本文なしの同期契約（sync.version 3）を要求し、音声送信は
recordingArchive.version 1 の Server に限定する。更新順序は Server / Web、Desktop とする（[同期契約](sync.md)）。

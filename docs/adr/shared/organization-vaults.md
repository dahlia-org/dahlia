...
Server Workspace は変更不能な Organization に所属する。Organization membership は内容へのアクセスを与えず、明示した user / organization / team permission だけを評価する。Workspace role は `admin | editor | viewer`、有効 role はこの順に強いものを採用する。Admin は内容と Workspace 自体・共有を管理し、Editor は内容の作成・更新・削除、録音、AI 処理、Local データの取り込みを行う。Viewer は read-only。Workspace の reset / 削除 / Server 間全内容移管は Admin のみ。
...
Better Auth の Organization role `owner | admin | member` と Workspace role は独立する。Header と accounts は Web の Better Auth セッション・Organization API を共用し、認証入口だけを変える。Header mode は全保護要求で検証済み proxy identity を要求し、cookie のみの認証や別 user の cookie を認めない。OAuth / Desktop / MCP の認証方式は維持する。
...
Header は初回登録時だけ、Organization の owner/admin が設定したドメインへ member として自動参加する。Organization の自動生成は行わず、未設定・共有メールドメインのユーザーも Personal で利用できる。複数ドメインを許可し、Organization 間の重複を拒否する。サブドメインは完全一致で別途指定する。共有メールドメインは固定版の公開リストで設定時と参加時に拒否する。DNS 所有確認は行わず設定権限を信頼する。Google は Personal 作成のみ。既存ユーザーの遡及参加と退会・除名後の再追加はしない。ドメイン設定の削除は所属を削除しない。2026-09-14 の更新で旧 `domain` 列を専用設定テーブルへ置き換え、既存設定は引き継がない。
...
Organization owner/admin の governance は配下 Workspace の ID / name / revision / creator ID と明示的な強制削除に限定し、内容を開示しない。削除確認後の変更は revision と変更 cursor で検出する。Server administrator は自動的に内容・governance 権限を得ない。

`created_by { id, name, email }` は作成時の不変な監査 snapshot。profile 変更で更新せず、Workspace 削除まで保持する。通常 API や検索には出さず、governance では creator ID だけを返す。account 削除 cleanup は未実装のため、自己削除・管理者削除とも拒否する。

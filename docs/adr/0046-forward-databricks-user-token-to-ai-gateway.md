...
- 管理画面のモデル発見は同じ user token で Unity Catalog Model Services API の `system.ai` schema を全ページ取得する。App は Responses 用の `ai-gateway` に加えて、発見用の `unity-catalog` user API scope を要求する。

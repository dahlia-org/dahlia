...
- A Vault's local export folder is optional, device-local state. It is never part of a Server transaction. Without a folder, Project, meeting, transcript, screenshot, and Summary records remain fully usable in SQLite and Server sync; Markdown export and filesystem watching are simply disabled.

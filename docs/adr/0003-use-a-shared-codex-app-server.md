...
- `thread/list` は `sourceKinds: ["vscode"]` と完全一致する `cwd` で絞り、`recency_at` の降順でページングする。固定版 v0.144.4 は app-server クライアントの thread を、固有の `originator` を持つこの source kind として保存する。

...
app-server protocol は発展中である。Dahlia は外部にインストールされた任意バージョンとの互換性を負わず、OpenAI Codex `0.144.4`、source commit `8c68d4c87dc54d38861f5114e920c3de2efa5876` を arm64 native binary として同梱する。この ADR はその固定版と現在の Dahlia 実装を対象とする。
...
要約固有の prompt、`SummaryDocumentResponse` schema、文字起こし、画像変換、response decode は `SummaryService` に残す。transport は業務データを理解せず、今後の Codex 機能も同じ connection と dispatcher を再利用する。
...
要約は tool を利用しない設計なので、approval request は次のように fail-closed で処理する。
...
### turn notification と要約生成

turn の状態は `(threadId, turnId)` を key にし、複数の要約や将来の Codex 操作を順不同で多重化する。

- `turn/start` の response より先に notification が到着する競合に備え、waiter 登録前の message を key ごとに最大 100 件保持する。
...
- 任意の利用箇所が進行通知を購読できるよう、turn key ごとの `AsyncStream` subscriber を提供し、turn completion または接続停止で必ず finish する。
...
- stdout JSONL の本文、文字起こし、画像 data URI、生成結果をログへ出さない。
...
- transcript、画像、prompt、生成結果を診断ログや Sentry へ送らない。
...
- Codex version 更新時は Rust build、protocol 差分、認証、model shape、notification の回帰確認が必要になる。
...
却下。Node/Bun runtime と package tree が追加で必要になる。Apple Silicon 向け native binary を Cargo で固定 build する方が bundle、署名、起動境界を単純にできる。
...
- OpenAI Codex app-server README（同梱 source commit）: <https://github.com/openai/codex/blob/8c68d4c87dc54d38861f5114e920c3de2efa5876/codex-rs/app-server/README.md>
- OpenAI Codex app-server protocol source: <https://github.com/openai/codex/tree/8c68d4c87dc54d38861f5114e920c3de2efa5876/codex-rs/app-server-protocol>

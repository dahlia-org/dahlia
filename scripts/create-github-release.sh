...
its signed and notarized DMG. The default path is Dahlia-<version>.dmg.
EOF
}
...
EXPECTED_DMG_NAME="${APP_NAME}-${MARKETING_VERSION}.dmg"
case "${1:-}" in
    "") DMG_PATH="${PROJECT_DIR}/${EXPECTED_DMG_NAME}" ;;
    /*) DMG_PATH="$1" ;;
    *) DMG_PATH="${INVOCATION_DIR}/$1" ;;
esac
...
if [ "$(basename "$DMG_PATH")" != "$EXPECTED_DMG_NAME" ]; then
    echo "error: DMG filename must match the app version: ${EXPECTED_DMG_NAME}" >&2
    exit 1
fi

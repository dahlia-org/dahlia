...
embed_embedding_dependency_licenses() {
    ...
    local notices=(
        ...
        "mlx-swift:LICENSE"
        "mlx-swift:ACKNOWLEDGMENTS.md"
        "mlx-swift-lm:LICENSE"
        "mlx-swift-lm:ACKNOWLEDGMENTS.md"
    ...
    for entry in "${notices[@]}"; do
        checkout="${entry%%:*}"
        notice="${entry#*:}"
        if [ ! -f "${project_dir}/.build/checkouts/${checkout}/${notice}" ]; then
            echo "error: ${checkout} ${notice} was not found in the SwiftPM checkout" >&2
            return 1
        fi
        mkdir -p "${contents_dir}/Resources/Licenses/${checkout}"
        cp \
...
embed_mlx_metal_library() {
    local project_dir="$1"
    local contents_dir="$2"
    local mlx_source="${project_dir}/.build/checkouts/mlx-swift/Source/Cmlx"
    local bundle_dir="${contents_dir}/Resources/mlx-swift_Cmlx.bundle"
    ...
    local work_dir
    local metal_sources=(
        "mlx-generated/metal/steel/attn/kernels/steel_attention.metal"
        "mlx-generated/metal/arg_reduce.metal"
        "mlx-generated/metal/conv.metal"
        "mlx-generated/metal/rms_norm.metal"
        "mlx-generated/metal/random.metal"
        "mlx-generated/metal/scaled_dot_product_attention.metal"
        "mlx-generated/metal/gemv.metal"
        "mlx-generated/metal/layer_norm.metal"
        "mlx-generated/metal/rope.metal"
    )

    if [ ! -d "$mlx_source" ]; then
        echo "error: mlx-swift checkout was not found" >&2
        return 1
    fi
    metal_toolchain="$(
        DEVELOPER_DIR="$developer_dir" xcodebuild -showComponent MetalToolchain -json 2>/dev/null \
            | plutil -extract toolchainIdentifier raw - 2>/dev/null || true
    )"
    if [ -z "$metal_toolchain" ]; then
        echo "error: MLX Metal shaders require: xcodebuild -downloadComponent MetalToolchain" >&2
        return 1
    fi

    staging_dir="$(mktemp -d "${TMPDIR:-/tmp}/dahlia-mlx-metal.XXXXXX")"
    work_dir="${staging_dir}/air"
    cp -R "${mlx_source}/mlx-generated" "${staging_dir}/mlx-generated"
    mkdir -p "$work_dir" "$bundle_dir"
    for metal_source in "${metal_sources[@]}"; do
        output_name="$(basename "$metal_source" .metal)"
        if ! DEVELOPER_DIR="$developer_dir" TOOLCHAINS="$metal_toolchain" xcrun -sdk macosx metal \
            -c "${staging_dir}/${metal_source}" \
            -I "${staging_dir}/mlx-generated" \
            -Wno-c++20-extensions -std=metal4.0 \
            -o "${work_dir}/${output_name}.air"; then
            rm -rf "$staging_dir"
            echo "error: MLX Metal shaders require: xcodebuild -downloadComponent MetalToolchain" >&2
            return 1
        fi
    done
    if ! DEVELOPER_DIR="$developer_dir" TOOLCHAINS="$metal_toolchain" xcrun -sdk macosx metallib \
        "${work_dir}"/*.air -o "${bundle_dir}/default.metallib"; then
        rm -rf "$staging_dir"
        return 1
    fi
    rm -rf "$staging_dir"

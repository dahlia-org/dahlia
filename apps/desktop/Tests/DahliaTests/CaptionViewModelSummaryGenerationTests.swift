...
@testable import DahliaRuntimeSupport
    ...
    struct CaptionViewModelSummaryGenerationTests {
        ...
        @Test
        func retiredArtifactHistoryDoesNotPreventLoadingSummary() async throws {
            let fixture = try SummaryGenerationFixture()
            defer { fixture.removeFiles() }
            let repository = MeetingRepository(dbQueue: fixture.database.dbQueue)
            let document = SummaryDocument(title: "Saved summary", sections: [])
            let meetingId = fixture.first.id
            let url = "https://dahlia.example/api/v1/artifacts/019cc4dd-e5c5-7bd4-94e0-98df9cc40db9"
            try repository.applyGeneratedSummary(toMeetingId: fixture.first.id, document: document, tags: [])
            try await fixture.database.dbQueue.write { db in
                try SummaryExportRecord.setURL(url, meetingId: meetingId, type: .dahliaArtifact, in: db)
            }
            let viewModel = CaptionViewModel()
            await fixture.select(fixture.first, in: viewModel, note: "note")
            #expect(await waitUntil { viewModel.currentSummaryDocument?.title == document.title })
            let exports = try repository.fetchMeetingDetail(id: fixture.first.id).summaryExports
            #expect(exports.first(where: { $0.type == .dahliaArtifact })?.url == url)
        }

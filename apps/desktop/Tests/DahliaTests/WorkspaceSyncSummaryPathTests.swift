...
@testable import DahliaRuntimeSupport
    ...
    @MainActor
    struct WorkspaceSyncSummaryPathTests {
        @Test
        func markdownRenameUpdatesStoredSummaryPathWithoutReadingFrontmatter() throws {
            ...
            let originalURL = context.workspaceURL.appending(path: "Project/Original.md")
            let renamedURL = context.workspaceURL.appending(path: "Project/Renamed.md")
            ...
            #expect(workspaceExport?.url == "vault:///Project/Renamed.md")
            #expect(workspaceExport?.workspaceRelativePath == "Project/Renamed.md")
        }
        ...
        func multipleMarkdownRenamesNeverInventPathPairings() throws {
            ...
            let projectURL = context.workspaceURL.appending(path: "Project", directoryHint: .isDirectory)
            ...
            #expect(try context.repository.fetchSummaryWorkspaceRelativePath(forMeetingId: context.meeting.id) == nil)
            #expect(try context.repository.fetchSummaryWorkspaceRelativePath(forMeetingId: second.id) == nil)
        }
        ...
        func laterBatchProcessesAfterRetryExhaustion() throws {
            ...
            #expect(try context.repository.fetchSummaryWorkspaceRelativePath(forMeetingId: context.meeting.id) == "Project/Renamed.md")
        }
        ...
        func markdownRenameThroughParentSymlinkClearsUnsafeStoredPath() throws {
            let context = try makeContext(projectName: "Project", summaryRelativePath: "Project/Original.md")
            let originalURL = context.workspaceURL.appending(path: "Project/Original.md")
            let outsideDirectory = context.workspaceURL.deletingLastPathComponent()
            ...
            let linkURL = context.workspaceURL.appending(path: "Link", directoryHint: .isDirectory)
        ...
        func directoryRenameUpdatesOnlyStoredSummaryPath() throws {
            ...
            let originalURL = context.workspaceURL.appending(path: "Original", directoryHint: .isDirectory)
            let renamedURL = context.workspaceURL.appending(path: "Renamed", directoryHint: .isDirectory)
            try FileManager.default.moveItem(at: originalURL, to: renamedURL)
            ...
            #expect(workspaceExport?.url == "vault:///Renamed/Summary.md")
            #expect(workspaceExport?.workspaceRelativePath == "Renamed/Summary.md")
        }
        ...
        func multipleDirectoryRenamesNeverInventPrefixPairings() throws {
            ...
            defer { try? FileManager.default.removeItem(at: context.workspaceURL) }
            let secondProjectURL = context.workspaceURL.appending(path: "Second", directoryHint: .isDirectory)
            ...
            let firstOld = context.workspaceURL.appending(path: "First", directoryHint: .isDirectory)
            let secondOld = secondProjectURL
            let firstNew = context.workspaceURL.appending(path: "First Renamed", directoryHint: .isDirectory)
            let secondNew = context.workspaceURL.appending(path: "Second Renamed", directoryHint: .isDirectory)
        ...
        private func makeContext(
            ...
            try FileManager.default.createDirectory(
                at: workspaceURL.appending(path: projectName, directoryHint: .isDirectory),
            ...
            let project = try repository.fetchOrCreateProject(name: projectName, workspaceId: workspace.id)

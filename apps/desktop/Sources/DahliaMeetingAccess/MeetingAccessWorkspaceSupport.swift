...
/// Workspace を跨いで書き込みを行うストアが共有する土台。
...
extension MeetingAccessStore {
    ...
    func withWorkspaceMutationLock<T>(workspaceURL: URL?, operation: () throws -> T) throws -> T {
    ...
    /// `summary_exports.url` に保存される Workspace 相対パス表現を読み解く。
    func workspaceRelativeSummaryPath(_ value: String) -> String? {
    ...
    func workspaceSummaryURL(_ relativePath: String) -> String {
    ...
    func isInsideWorkspace(_ value: URL, workspaceURL: URL) -> Bool {

...
extension DahliaMCPServer {
    ...
    func executeTool(named name: String, arguments: [String: Any]) throws -> [String: Any] {
        ...
        let definitions = Self.readOnlyToolDefinitions + Self.liveToolDefinitions
        guard let definition = definitions.first(where: { $0["name"] as? String == name }) else {
            // Writes retain the original, explicitly configured single-Vault boundary.
            guard vaultScope != nil else { throw ParameterError("Unknown read tool") }
            return try executeScopedTool(named: name, arguments: arguments)
        }
        ...
        if name.hasPrefix("get_") {
            let mapping: [String: (String, String)] = [
                ...
                "get_live_transcript": ("meeting_id", "meetings"),
        ...
        for vault in try vaults() {
            do {
                ...
                let code = (error as? MeetingAccessError)?.reasonCode ?? (error as? LiveTranscriptError)?.rawValue ?? (error as? TextContentError)?
    ...
    private func scopedRead(name: String, arguments: [String: Any], vaultID: UUID) throws -> [String: Any] {
        ...
        if name == "list_live_meetings" || name == "get_live_transcript" {
            return try server.liveTool(name: name, arguments: arguments)
        }
        return try server.executeScopedTool(named: name, arguments: arguments)
    }

    var workspaceToolDefinitions: [[String: Any]] {
        let reads = (Self.readOnlyToolDefinitions + Self.liveToolDefinitions).map { definition in
            var definition = definition
            var schema = definition["inputSchema"] as? [String: Any] ?? [:]
            var properties = schema["properties"] as? [String: Any] ?? [:]
            var vaultSchema = Self.idSchema(.vault)
            vaultSchema["description"] = "Optional Vault filter; cannot widen the configured Vault scope."
            properties["vault_id"] = vaultSchema
            schema["properties"] = properties
            definition["inputSchema"] = schema
            if vaultScope == nil, (definition["name"] as? String)?.hasPrefix("get_") != true {

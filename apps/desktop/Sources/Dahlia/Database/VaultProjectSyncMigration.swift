...
enum VaultProjectSyncMigration {
    ...
    WHEN new.syncEnabled = 1 BEGIN
        INSERT INTO vault_sync_jobs(vaultId) VALUES(new.id)
        ON CONFLICT(vaultId) DO UPDATE SET generation = generation + 1, status = 'pending', attempts = 0,
    ...
    WHEN old.syncEnabled = 0 AND new.syncEnabled = 1 BEGIN
        ...
        ON CONFLICT(vaultId) DO UPDATE SET generation = generation + 1, status = 'pending', attempts = 0,
            ...
            lastErrorCode = NULL, updatedAt = unixepoch('subsec');
    END;
    CREATE TRIGGER vault_sync_queue_project_insert AFTER INSERT ON projects BEGIN
    ...
    AFTER UPDATE OF vaultId, parentProjectId, name, description, projectType, revision ON projects BEGIN
        ...
        INSERT INTO vault_sync_jobs(vaultId) SELECT new.vaultId WHERE EXISTS (
            ...
            lastErrorCode = NULL, updatedAt = unixepoch('subsec');
    END;
    CREATE TRIGGER vault_sync_queue_project_delete AFTER DELETE ON projects BEGIN
        INSERT INTO vault_sync_jobs(vaultId) SELECT old.vaultId WHERE EXISTS (
            SELECT 1 FROM vaults WHERE id = old.vaultId AND syncEnabled = 1
        ) ON CONFLICT(vaultId) DO UPDATE SET generation = generation + 1, status = 'pending', attempts = 0,
            availableAt = unixepoch('subsec'), claimedAt = NULL, leaseExpiresAt = NULL,
    ...
    AFTER UPDATE OF projectId ON meetings BEGIN

# Deploy Dahlia Cloud on Databricks Apps

This target uses the checked-in `DAHLIA_RUNTIME=databricks` preset: Databricks Apps header identity and PostgreSQL/Lakebase. The upstream uses the same OpenAI-compatible contract as every runtime. The Apps proxy authenticates browser and U2M requests before they reach Dahlia Cloud, so no Better Auth session is created.
...
Databricks Apps proxy
        │ X-Forwarded-Email
        ▼
Dahlia Cloud App ─┬─ OPENAI_API_KEY ── OpenAI-compatible Responses API
...
Databricks runs the root `build` script during deployment. The startup command then applies the PostgreSQL migration under an advisory lock and starts the cloud package. Header authentication does not create sessions, but the shared schema contains Gateway administration tables.

## 4. Upload and deploy

```bash
databricks sync . /Workspace/Users/<you>/dahlia-cloud
databricks apps deploy <app-name> \
  --source-code-path /Workspace/Users/<you>/dahlia-cloud
```
...
- Provider credentials live in Databricks-managed app secrets and environment variables; Dahlia Cloud does not persist them.

...
function stripeEnv(): Record<string, string> {
  return {
...
describe("configuration", () => {
  ...
  it("enables Stripe only when all values are configured for accounts", () => {
    expect(loadConfig({ ...accountsEnv(), ...stripeEnv() }).stripe).toEqual({
      secretKey: "sk_test_secret",
      webhookSecret: "whsec_test",
      proMonthlyPriceId: "price_pro_monthly",
    });
    expect(loadConfig(accountsEnv()).stripe).toBeUndefined();
  });

  it("rejects partial Stripe configuration", () => {
    expect(() => loadConfig({ ...accountsEnv(), STRIPE_SECRET_KEY: "sk_test_secret" }))
      .toThrow("must be configured together");
  });

  it("rejects Stripe in header authentication", () => {
    expect(() => loadConfig({ ...headerEnv(), ...stripeEnv() }))
      .toThrow("requires DAHLIA_AUTH_PROVIDER=accounts");
  });

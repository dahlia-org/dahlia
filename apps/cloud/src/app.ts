...
interface AppDependencies {
  ...
  auth?: DahliaAuth;
  authStore?: AuthStore;
  billing?: Pick<BillingService, "canUseGateway" | "summary">;
}
...
export function personalBillingMutationAllowed(
  path: string,
  body: BillingMutation,
  userId: string,
): boolean {
  if (body.referenceId !== undefined && body.referenceId !== userId) return false;
  if (body.customerType !== undefined && body.customerType !== "user") return false;
...
export function createApp(dependencies: AppDependencies) {
  ...
  const billing = config.stripe
    ? dependencies.billing ?? new BillingService(config.stripe, authStore!)
    : undefined;
  ...
  app.use("/api/auth/subscription/*", async (context, next) => {
    if (!billing || !auth) return next();
    const session = await auth.api.getSession({ headers: context.req.raw.headers });
    if (!session) return context.json({ error: "unauthorized" }, 401);
    const body = await context.req.raw.clone().json().catch(() => null) as BillingMutation | null;
    if (body && !personalBillingMutationAllowed(context.req.path, body, session.user.id)) {
      return context.json({ error: "billing_reference_forbidden" }, 403);
    }
    await next();
  });
  ...
  app.get("/api/session", (context) => {
    ...
    return Promise.resolve(isAdministrator(config, store, identity)).then((admin) => context.json({
      capabilities: {
        admin,
  ...
  if (billing) {
    app.use("/api/billing/*", async (context, next) => {
      context.set("identity", await identities.fromBrowser(context.req.raw, remoteAddress(context)));
      await next();
    });
    app.get("/api/billing/summary", async (context) => {
      return context.json(await billing.summary(context.get("identity").userId));
    });
  }
  ...
  app.use("/api/v1/*", async (context, next) => {
    ...
    if (billing && !await billing.canUseGateway(context.get("identity").userId)) {
      return context.json({ error: "billing_required" }, 402);
    }
    await next();
  });

...
import { resolveDashboardRoute, shouldRedirectToSignIn } from "./routes";
...
function Brand() {
  return (
    <a className="brand" href="/dashboard" aria-label="Dahlia Cloud home">
    ...
    <main className="auth-page">
      <section className="auth-card">
        ...
        <div className="auth-copy">
          ...
          <p>Audio, transcripts, and local recordings stay on your Mac. Dahlia Cloud only brokers Codex requests.</p>
        </div>
...
function Shell({ session, children }: { session: SessionInfo; children: React.ReactNode }) {
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Account navigation">
          ...
          {session.capabilities.billing && (
            <a className={route === "/dashboard/billing" ? "active" : ""} href="/dashboard/billing">
              Billing &amp; Invoices
            </a>
          )}
...
function Billing() {
  ...
  const load = useCallback(() => {
    setError(undefined);
    void json<BillingSummary>("/api/billing/summary").then(setSummary).catch((caught: Error) => setError(caught.message));
  }, []);
  useEffect(load, [load]);
  ...
  async function upgrade() {
    ...
    try {
      ...
      if (result.error) setError(result.error.message || "Could not start checkout");
    } catch (caught) {
  ...
  async function openPortal() {
    ...
    setError(undefined);
    try {
      const result = await authClient.subscription.billingPortal({ returnUrl: "/dashboard/billing" });
      if (result.error) setError(result.error.message || "Could not open billing management");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Could not open billing management");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <>
      ...
      {!summary && !error && <section className="panel"><p className="muted">Loading billing…</p></section>}
      {summary && (
        <>
          <section className="section-block">
            <h2 className="section-label">Current plan</h2>
            <div className="panel billing-card current-plan">
              <div>
                <div className="plan-heading">
                  <h3>{summary.plan === "pro" ? "Pro" : "Free"}</h3>
                  <span className={`status ${summary.status === "active" || summary.status === "trialing" ? "good" : ""}`}>
                    {summary.status.replaceAll("_", " ")}
                  </span>
                </div>
                <p className="plan-price">
                  {formatMoney(summary.monthlyAmount, summary.currency)}
                  <span> / month</span>
                </p>
                {summary.renewsAt && <p className="billing-note">Your subscription renews on {formatDate(summary.renewsAt)}.</p>}
                {summary.endsAt && <p className="billing-note">Your subscription ends on {formatDate(summary.endsAt)}.</p>}
                {summary.plan === "free" && <p className="billing-note">Upgrade to use the Dahlia AI Gateway.</p>}
              </div>
              {summary.plan === "free" ? (
                <button className="primary" disabled={pending} onClick={() => void upgrade()}>Upgrade</button>
              ) : (
                <button className="secondary" disabled={pending} onClick={() => void openPortal()}>Manage plan</button>
              )}
            </div>
          </section>
          ...
          <section className="section-block">
            ...
            <div className="panel billing-card payment-card">
              <p>Update your payment details and billing information.</p>
              {summary.paymentManagementAvailable ? (
                <button className="secondary" disabled={pending} onClick={() => void openPortal()}>Manage in Stripe</button>
              ) : <span className="muted">Available after upgrading</span>}
            </div>
          </section>

          <section className="section-block">
            <h2 className="section-label">Invoices</h2>
            <div className="panel invoices-card">
              {summary.invoices.length === 0 ? (
                <div className="empty-state"><strong>No invoices yet</strong><span>Paid invoices will appear here.</span></div>
              ) : (
                  ...
                  <table>
                    ...
                    <tbody>
                      {summary.invoices.map((invoice, index) => (
                        <tr key={`${invoice.createdAt}-${index}`}>
                          <td>{formatDate(invoice.createdAt)}</td>
                          <td>{invoice.description}</td>
                          <td><span className={`status ${invoice.status === "paid" ? "good" : ""}`}>{invoice.status}</span></td>
                          ...
                          <td className="invoice-links">
                            ...
                            {invoice.invoicePdfUrl && <a href={invoice.invoicePdfUrl} target="_blank" rel="noreferrer">PDF</a>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
...
export function App() {
  ...
  if (!session) return <main className="loading"><Brand /><span>Loading account…</span></main>;
  const route = resolveDashboardRoute(path, session.capabilities);
  if (route.redirect) {
  ...
  if (route.page === "admin-models") page = <AdminModels />;
  else if (route.page === "admin-members") page = <AdminMembers />;
  else if (route.page === "billing") page = <Billing />;

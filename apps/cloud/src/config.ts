...
function stripeConfig(
  ...
  const values = {
    ...
    proMonthlyPriceId: env.STRIPE_PRO_MONTHLY_PRICE_ID?.trim(),
  };
  ...
  if (authProvider !== "accounts") {
    throw new Error("Stripe billing requires DAHLIA_AUTH_PROVIDER=accounts");
  }
...
export function loadConfig(env: Record<string, string | undefined>): AppConfig {
  ...
  const config: AppConfig = {
    ...
    stripe: stripeConfig(env, authProvider),

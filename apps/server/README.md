...
The account menu's **Connect with MCP** dialog shows `mcp.json`, Claude Code, and Codex setup using the canonical application origin above. When `DATABRICKS_APP_URL` is present in Header authentication, it generates `uvx uc-mcp-proxy` commands with a selectable Databricks CLI profile. Accounts-mode Node deployments connect directly to the Server's Streamable HTTP MCP endpoint through CIMD. Accounts-mode Cloudflare Workers and non-Databricks Header deployments do not advertise setup because they do not provide an authenticated remote MCP transport.

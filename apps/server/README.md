...
- `GET .../summary/versions`: `{ items, nextCursor }`, newest revision first, without document bodies. `limit` defaults to 20 (maximum 100); `cursor` is the exclusive revision boundary returned by the previous page. Items include `revision`, `title`, `createdAt`, `savedAt`, and nullable `metadata`.
- `GET .../summary/versions/{revision}`: the version metadata and document body, or 404.

Desktop fetches Server summary bodies through `latest`, checks their revision against synchronized metadata and verifies the existing content digest. A racing update triggers synchronization and one fresh read; it never mixes revisions or overwrites pending local edits. The legacy revision-checked text endpoint remains supported. Web reads meeting metadata with `content=metadata-v1` and the current summary body only through `latest`, avoiding duplicate body downloads on live refresh. It allows read-only historical selection; edits apply to the current summary. Deleting a summary removes **all** its versions atomically; deleting its meeting or Vault also removes the versions. The summary revision counter is not reset by summary deletion.

...
`GET /api/v1/meetings/{meetingId}/transcripts/{version}/conversation-analytics` synchronously calculates owner-only analytics for one immutable transcript version. The response includes the transcript ID/version, calculation version, metrics, and bounded chart intervals. Missing committed recording audio returns `200` with `status: "unavailable"`; an inaccessible meeting or Vault, a member Vault, and an absent transcript version return `404`. Desktop does not calculate or cache these values locally.

...
The Worker-safe package root exports the backend extension contract from `dahlia-ai`; Node-only APIs such as `createNodeAuthStore` are exported from `dahlia-ai/node`. Dashboard components come from `dahlia-ai/client`, shared styles from `dahlia-ai/client/styles.css`, and the migration manifest from `dahlia-ai/migrations`. Server migrations must run before consumer migrations. Give every SQLite and PostgreSQL migration directory a stable lowercase ledger ID; never derive it from manifest position. Each SQLite directory explicitly lists the SQL filenames to apply; other files in that directory are ignored.

...
`DAHLIA_LOCAL_SINGLE_USER=1` runs Header mode without a proxy, as Omnigent's bare local server does. It replaces the identity header with the fixed `local@example.com` address: `DAHLIA_AUTH_HEADER` is not read at all, so a client-supplied copy cannot select another identity, and `X-Forwarded-Preferred-Username` is ignored as well. This is a substitution in the header parsing step only; validation, normalization, `account.account_id` storage, JIT projection to an internal UUID, Personal Organization setup, and Better Auth Header sign-in are the same as any other Header deployment. Every request therefore authenticates as that single user, which removes authentication entirely, so enable it only on a machine that is not reachable by anyone else. Any value other than `1` leaves it off, and `accounts` mode never reads the setting.

...
Automatically created Personal Organization slugs use the email local part; Header-created Team Organization slugs use the domain. Both lowercase the source and replace each non-ASCII-alphanumeric character with `_` (for example, `kazuki.matsuda` becomes `kazuki_matsuda` and `example.co.jp` becomes `example_co_jp`). An empty source in local single-user mode uses `organization`. Collisions receive `_2`, `_3`, etc. inside the authorization transaction. Existing slugs are unchanged. Organization Settings pairs each name or slug value with its edit action in a responsive row and separates leaving/deleting the organization from general settings. The Personal owner or Team owner/admin can change the slug through the existing organization update API. Manual slugs accept lowercase ASCII letters, digits, `_`, and `-`; empty, invalid, duplicate, and Team `personal-` reserved slugs are rejected. Organization URLs use `/orgs/{organizationId}` and remain unchanged when the slug is edited. IDs, domains, and membership remain unchanged.

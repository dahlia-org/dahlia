...
Restoring a Vault after a reset with `preservePermissions: true` requires Vault Admin. A user without Admin cannot restore a revision-zero Vault; inaccessible or reused IDs return non-retryable errors without publishing data.
...
`GET /api/v1/meetings/{meetingId}/transcripts/{version}/conversation-analytics` synchronously calculates Admin/Editor analytics for one immutable transcript version. The response includes the transcript ID/version, calculation version, metrics, and bounded chart intervals. Missing committed recording audio returns `200` with `status: "unavailable"`; an inaccessible meeting or Vault, a Viewer Vault, and an absent transcript version return `404`. Desktop does not calculate or cache these values locally.
...
Receipt bodies are retained for 90 days. Transaction ID, actor, Vault, request hash, result IDs/revisions and commit cursor remain as durable idempotency records; account deletion is disabled. Canonical meeting data has no new retention limit; lightweight receipt storage is not constant-sized.
...
Organization, Team, and direct User grants use `admin | editor | viewer`. Admins manage Vault settings, sharing, reset, deletion, and whole-Vault transfer. Admins and Editors create, update, and delete content, record, and request AI processing. Viewers can read. Both ends of a Server transfer require Admin; Local imports into existing Vaults also allow Editor.

Both auth modes use Better Auth Web sessions, Organizations, invitations, and Teams. Header requests still require verified proxy identity and reject a mismatched session user. Every new user receives a Personal Organization and one private Personal Vault. Header users also join the Organization matching their email domain on first registration. Native administrator user creation in Header mode atomically provisions the same Header identity and Organizations, so subsequent proxy sign-in retains the created user and its grants. Header user emails cannot be edited through Better Auth: a changed proxy email represents a separate identity. The first user owns the Organization and later users are members. Departure or removal is not undone at sign-in. Domain Organizations can be deleted under ordinary Team rules; Personal sharing/deletion is prohibited. Organization/team membership and permission changes are validated atomically, including concurrent attempts to remove the last effective Admin. Account deletion is disabled.
...
`pnpm dev` / `pnpm dev:api` enables sample data for local SQLite when `DAHLIA_APP_URL` uses localhost or a loopback address. After normal sign-in, a Personal Vault without synchronization history receives a Project and three meetings with summaries, including an unclassified meeting. Existing content and history are left untouched; repeated requests and restarts do not duplicate the sample. The normal transaction service handles serialization and search indexing. PostgreSQL, Lakebase, Workers, and `pnpm start` do not seed data. Sample content can be removed through ordinary content actions; Personal Vault itself cannot be deleted. Google sign-in configuration is required only in accounts mode.
...
The first registered user becomes the Server administrator. In both auth modes, user creation atomically initializes their Personal Organization, owner membership, Personal Vault and direct Vault Admin permission. Header registration looks up the normalized email domain in the unique, nullable Better Auth Organization custom field `domain`, creating a Team Organization if absent. The first user is its owner and later users are members. The domain is immutable; names can change. Ordinary and Personal Organizations have a null domain. Enrollment occurs only on initial registration, so departure/removal persists across sign-ins and restarts. Domain Organizations follow normal Team deletion rules; a later new registration can recreate a deleted domain Organization. Domains match exactly, including subdomains and public email providers. Google registration creates only Personal resources. Organization membership does not grant Vault access. Account deletion is disabled.
...
Meetings retain their readable Summary, Screenshots and Transcript tabs, breadcrumbs and actions allowed by the effective Vault role.
...
Vault Admins can delete an empty Team Vault from its Settings tab after confirmation. Vault detail includes `hasResources`; the client disables deletion while resources remain. The revision-checked `vault:reset` transaction with `preservePermissions: false` also rejects Projects, Meetings, or Files (including staged Files) with `409 vault_not_empty`, then returns to Vaults after a committed receipt. The Admin-only restore operation with `preservePermissions: true` retains its reset semantics.
...
Identical Vaults return 400; missing or inaccessible Vaults return 404. Stale revisions, staged data, running work,
...
Project details use Meetings and Settings tabs. Project editing and deletion are available to Vault Admins and Editors from Settings.
...
`embedding_text` is not stored. Jobs use the existing tokenized `search_text` projection, including meeting title, tags, description and summary or image OCR and caption. The existing `embedding_content_hash` is the hash of this exact input. A changed input clears the vector and its model atomically with the projection update. Saving requires the latest hash, the claimed job generation/model and Vault scope to match; a deleted document cannot be recreated by a delayed result. Queries require the configured model and vector length, so model changes fall back to FTS until regeneration. No Meeting/image version or second vector hash is added.

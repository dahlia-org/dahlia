...
`GET /api/v1/workspaces` returns every accessible Workspace with required `organizationId` and effective `role: admin | editor | viewer`. Optional `organizationId=org_…` filters by the owning Organization. Organization membership alone grants no Workspace access. The retired owner query is rejected.
...
Home shows the accessible Workspaces and the ten most recent meetings from a selected Workspace.

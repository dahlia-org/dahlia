...
Authenticated users read `GET /api/v1/account/settings` and PATCH `analysisLanguages: { scope: "all" | "selected", identifiers: [...] }`. Scope and identifiers update atomically; selected requires at least one identifier. GET returns `{ settings: null }` before initialization. Desktop conditionally initializes the recognition languages using `initialize: true`. The existing `account_settings` SSE invalidation triggers refetches. Recognition languages remain personal; output language belongs to the Workspace.

Workspace `generationSettings` contains the shared output language, summary style, processing location, remote workflow/models/effort, and local model/effort. Administrators update the full object through the existing Workspace transaction with `baseRevision`; members can read it. Receipts, deltas, snapshots and SSE use the existing Workspace sync path. Desktop persists the same object for local and Server Workspaces, so cached settings remain available offline and recording never waits for a settings fetch. No personal generation preferences are backfilled.
...
The table keeps `user_id`, `output_language`, `analysis_languages`, `summary`, `processing`, and internal `revision`. PostgreSQL uses JSONB; SQLite use JSON text. Atomic partial updates increment revision only when supplied values differ. SSE polls this revision every two seconds; clients refetch on account-settings notifications and reconnect without reloading the model catalog or unrelated Web data.
...
Both authentication modes allow users to create Organizations explicitly. Members can reach their organization settings from the sidebar; administrators use "Manage your organizations" on the organization directory.
...
Update Desktop before enabling audio generation on Server, and apply the new PostgreSQL/SQLite `audio_summary` migration before updating Server and Web. Desktop displays and edits the selected source settings, filters audio models to audio-capable Gemini, and uses the common detail for individual and batch generation. Workers advertise generation methods when a supported provider, summary Queue and Images binding are configured. Custom Node entry points should register both available factories with the existing service and worker. No Google API key or additional dependency is required.
...
Workspace selection and management live in the sidebar; the account menu contains account and organization actions.

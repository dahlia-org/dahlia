...
The read-only MCP tools `list_live_meetings` and `get_live_transcript` use current Vault sharing permissions. Only confirmed speech received through ordinary transcript sync is returned. Recording state comes from the existing `recording_sessions` view; unconfirmed previews remain in Local MCP memory, with no upload API or dedicated table. HTTP reads and the separate `/api/v1/meetings/{meetingId}/live-transcript/events` SSE route share the same service. Reconnect with `Last-Event-ID`, rebuild on `reset`, and stop on `error`. Slow subscribers are disconnected after a five-second stalled write. Existing sync SSE is unchanged.

Recording status reflects synchronized start/end events, not live connectivity. An offline recording remains active until its end event synchronizes. `confirmedState` and `confirmedThrough` describe the independent confirmed-data sync status. See [ライブ MCP / English quick reference](../../docs/live-mcp.md) for registration, polling, status semantics, and endpoints.

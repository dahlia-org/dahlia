...
Private Web uses Mastra Message History when the application database is PostgreSQL-compatible. Threads are owned by the authenticated user's TypeID, fixed to their creation Workspace, and protected by FORCE RLS in the separate `agent` schema. Agent RLS derives that user TypeID from the same transaction-local `app.user_id` used by the application schemas; no separate Agent identity setting is accepted. A user can list, reopen, continue, and delete their own threads; current Workspace permission is still checked by every meeting tool call. Removing Workspace access does not redact text already persisted in that user's thread, and the user can continue using that text as model context; delete the thread to remove it. Server-encrypted Workspaces keep page-only chat because their derived prompts and answers may not be duplicated as plaintext in `agent`. SQLite also keeps page-only chat and exposes no history routes.

...
Roll out `GET /api/v1/capabilities` (version 1) before updating Desktop. It uses existing authenticated sync capabilities and remains unavailable on stores without atomic sync. Older clients keep their default full representations and transaction schema version 2. New clients must fail closed with an update-required state when this capability or the `metadata-v1` response marker is missing.

...
Both auth modes use Better Auth Web sessions, Organizations, invitations, and Teams. Header requests still require verified proxy identity and reject a mismatched session user. Every new user receives a Personal Organization and one private Personal Workspace. Header users join an explicitly configured Organization as members on first registration when their email domain matches. Native administrator user creation in Header mode atomically provisions the same Header identity and Organizations, so subsequent proxy sign-in retains the created user and its grants. Header user emails cannot be edited through Better Auth: a changed proxy email represents a separate identity. Organization creation is explicit; its creator becomes owner. Departure or removal is not undone at sign-in. Domain Organizations can be deleted under ordinary Team rules; Personal sharing/deletion is prohibited. Organization/team membership and permission changes are validated atomically, including concurrent attempts to remove the last effective Admin. Account deletion is disabled.
...
The first registered user becomes the Server administrator. In both auth modes, user creation atomically initializes their Personal Organization, owner membership, Personal Workspace and direct Workspace Admin permission. Header registration joins an existing Team Organization only when its auto-join settings contain the normalized email domain. Organization Settings lets owners/admins set up to 50 domains; an empty list disables auto-join. The creator remains owner and automatic participants are always members. Existing users are not enrolled retroactively; departure/removal persists across sign-ins and restarts. Deleting or clearing settings does not remove members. Domain matching is exact: subdomains must be configured separately. Google registration creates only Personal.

`GET /api/v1/organizations/{organizationId}/auto-join-domains` returns `{ "domains": [] }` to members. `PUT` replaces the list and is limited to Team owners/admins, rejecting impersonation and cross-origin writes. Values are trimmed, lowercased and deduplicated; invalid hostnames, email addresses, URLs and wildcards are rejected. Domains are unique across Organizations (`409 auto_join_domain_in_use`); owner/admin authority is trusted without DNS verification. Shared email domains are rejected (`400 shared_email_domain`) using a pinned copy of [free-email-domains](https://github.com/Kikobeats/free-email-domains), including its listed subdomains. The same check applies at enrollment; affected users can still sign in and receive Personal. The list is bundled with its MIT license and revision notice; update the pinned data through review, with no runtime network lookup. Its coverage depends on that snapshot.

The unreleased Server initial migrations directly create `organization_auto_join_domains` without the old `organization.domain` field. Auto-join starts disabled until configured. This baseline requires an empty database; no conversion or automatic deletion of existing databases is performed.

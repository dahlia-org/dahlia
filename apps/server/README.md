...
Both auth modes use Better Auth Web sessions, Organizations, invitations, and Teams. Header requests still require verified proxy identity and reject a mismatched session user. Every new user receives a Personal Organization and one private Personal Workspace. Header users also join the Organization matching their email domain on first registration. Native administrator user creation in Header mode atomically provisions the same Header identity and Organizations, so subsequent proxy sign-in retains the created user and its grants. Header user emails cannot be edited through Better Auth: a changed proxy email represents a separate identity. The first user owns the Organization and later users are members. Departure or removal is not undone at sign-in. Domain Organizations can be deleted under ordinary Team rules; Personal sharing/deletion is prohibited. Organization/team membership and permission changes are validated atomically, including concurrent attempts to remove the last effective Admin. Account deletion is disabled.
...
`DAHLIA_LOCAL_SINGLE_USER=1` runs Header mode without a proxy, as Omnigent's bare local server does. `DAHLIA_AUTH_HEADER` still wins whenever it carries a value, including `X-Forwarded-Preferred-Username` for the display name; the fixed `local@example.com` address is only a fallback for a missing or blank header. The setting also relaxes validation so a value that is not an email, such as `garbage`, becomes the identity as written rather than being rejected. Such a value has no domain part, so it registers as `personal` and skips domain Organization creation and enrollment; its Personal Organization and Personal Workspace are created normally, and an address that does have a domain enrolls as usual. Everything else is a substitution in the header parsing step only: trimming, lowercasing, `account.account_id` storage, JIT projection to an internal UUID, and Better Auth Header sign-in are the same as any other Header deployment. Because no proxy strips a client-supplied header in this setup, anyone who can reach the Server can name any identity, so enable it only on a machine that is not reachable by anyone else. Any value other than `1` leaves it off, in which case a missing or invalid header is rejected exactly as before, and `accounts` mode never reads the setting.
...
The first registered user becomes the Server administrator. In both auth modes, user creation atomically initializes their Personal Organization, owner membership, Personal Workspace and direct Workspace Admin permission. Header registration looks up the normalized email domain in the unique, nullable Better Auth Organization custom field `domain`, creating a Team Organization if absent. The first user is its owner and later users are members. The domain is immutable; names and slugs can change. Ordinary and Personal Organizations have a null domain. Enrollment occurs only on initial registration, so departure/removal persists across sign-ins and restarts. Domain Organizations follow normal Team deletion rules; a later new registration can recreate a deleted domain Organization. Domains match exactly, including subdomains and public email providers. Google registration creates only Personal resources. Organization membership does not grant Workspace access. Account deletion is disabled.

Automatically created Personal Organization slugs use the email local part; Header-created Team Organization slugs use the domain. Both lowercase the source and replace each non-ASCII-alphanumeric character with `_` (for example, `kazuki.matsuda` becomes `kazuki_matsuda` and `example.co.jp` becomes `example_co_jp`). An empty source in local single-user mode uses `organization`. Collisions receive `_2`, `_3`, etc. inside the authorization transaction, using one lookup of occupied slugs and choosing the first vacant suffix in memory. Existing slugs are unchanged. Organization Settings pairs each name or slug value with its edit action in a responsive row and separates leaving/deleting the organization from general settings. The Personal owner or Team owner/admin can change the slug through the existing organization update API. Manual slugs accept lowercase ASCII letters, digits, `_`, and `-`; empty, invalid, duplicate, and Team `personal-` reserved slugs are rejected. Organization URLs use `/orgs/{organizationId}` and remain unchanged when the slug is edited. IDs, domains, and membership remain unchanged.

...
Screenshot `GET` / `HEAD` reads at `/api/v1/vaults/{vaultId}/meetings/{meetingId}/screenshots/{screenshotId}/content` accept `variant=thumbnail` or `variant=original` (the default). Node generates a 384px, quality-75, no-upscale WebP thumbnail with `sharp` after the original's canonical transaction commits, and retries on demand. The thumbnail is stored beside its immutable original using the existing object storage and durable deletion path. There is no separate medium-size preview: full-size viewing and exports read the original. Generation is bounded to two concurrent conversions and does not gate the commit response. Every read rechecks current Vault access, including cached thumbnails. Workers and failed conversions return the original; `X-Dahlia-Image-Variant` reports the actual representation and `X-Dahlia-Original-SHA256` binds it to its original. Native image processing is exported only through the Node entry point; package-root imports remain Worker-safe.

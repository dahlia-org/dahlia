...
For administrators, `GET /api/admin/models` lists the models available from `DATABRICKS_HOST/ai-gateway/mlflow/v1/models` and merges their saved enabled state. The Dashboard enables or disables those models directly; it does not show the manual Model Alias form for this backend.

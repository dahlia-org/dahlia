...
`GET /api/v1/models` uses the App service principal to list all pages of Model Services under the required `DATABRICKS_MODEL_SCHEMA` (`catalog.schema`, for example `dahlia.ai`; specify catalog and schema names in lowercase). Names containing `embedding` are excluded. When list rows include `supported_api_types`, only services supporting `mlflow/v1/responses` are exposed in the standard list and picker-visible Codex catalog. When that field is absent, discovery includes the remaining services without individual GETs. Operators must follow this naming convention for embedding services whose capabilities are omitted. Empty capabilities exclude a service; malformed capabilities return `502 provider_models_invalid`. The configured `CODEX_AUTO_REVIEW_MODEL` override is trusted without capability discovery. Model IDs are returned as short names such as `gpt-5-6-luna`; the Databricks backend prefixes that configured schema when forwarding Responses. Fully qualified model IDs from clients are rejected. The Server-controlled `CODEX_AUTO_REVIEW_MODEL` override is exempt and is sent unchanged. Databricks requires `DATABRICKS_CLIENT_ID` and `DATABRICKS_CLIENT_SECRET`; Apps injects both at runtime. Desktop authorization requests `all-apis`; the App keeps the `ai-gateway` and `files` OBO scopes.

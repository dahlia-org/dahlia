...
`recall_workspace_memory` and `reflect_workspace_memory` are AI-chat tools, not public MCP tools. Every read checks current Workspace permission and reloads/hash-checks canonical Dahlia sources. Only canonical excerpts are evidence; reflection is an explicitly unverified hypothesis and is omitted when its complete source lineage cannot be verified. Counts and exhaustive comparisons still require canonical queries. Retrieval is bounded to five sources and 16,000 characters per excerpt; the AI can retrieve full meeting data with existing tools. A transcript establishes what was recorded, not the external truth of a speaker's claim.
...
Durable coordination metadata in `jobs` survives Workspace deletion and holds only IDs, source revisions/hashes, cursors and operation state. Shared-note content lives in `app.shared_memories` with forced PostgreSQL RLS and equivalent SQLite authorization. Canonical sync changes and note edits invalidate the generation transactionally; reads resume after reconciliation. Updates replace deterministic documents, obsolete documents and derived models are deleted, and deletion of a Workspace schedules remote-bank cleanup. Retain intent is persisted before submission using Hindsight's idempotent operation ID. Leases, asynchronous completion polling and bounded retry backoff recover interrupted work without blocking canonical writes. Retain completion is required before a document is marked indexed.

Node runs an in-process polling worker. Cloudflare requires `DAHLIA_MEMORY_QUEUE`, its consumer and the existing scheduled trigger; queue payloads contain IDs only and durable state determines work. Apply the additive application migrations before starting either runtime. Do not repoint the URL/prefix until existing banks have been erased with the old configuration: external banks cannot be moved by changing environment variables.

Oversized meetings and asynchronous operations still failed/cancelled after three retries are skipped for that generation. Pending operations are never abandoned. Failed document projections are removed before reads resume. Settings expose partial completion, the skipped count and up to 20 source/model identifiers; tools disclose incomplete coverage. Correct the source or connection and retry to reconcile again. Transient transport/authentication and cleanup failures remain retryable and do not publish a completed generation.

Initial ceilings: one meeting per work step, a 4 MiB source-document limit (oversize is skipped visibly without silently truncating), one in-flight step per Workspace, and full reconciliation/model regeneration after source changes. Ongoing sync can keep a Workspace pending; incremental per-source reconciliation is the next step if this becomes frequent. No dependency was added.

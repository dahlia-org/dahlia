...
`GET/PATCH /api/v1/account/settings` adds `summary.method: "audio"` and `summary.methodSettings.audio: { model, reasoningEffort, detail }`. The audio defaults are `gemini-3-8-flash`, `medium`, and `detailed`; the existing default source remains `transcript`. Switching sources preserves each source's settings, and nested PATCH still changes only supplied fields. If the configured model is unavailable, select an available Gemini model. Output language and per-job detail overrides remain shared.
...
Update Desktop before enabling audio generation on Server, and apply the new PostgreSQL/SQLite/D1 `audio_summary` migration before updating Server and Web. Desktop displays and edits the selected source settings, filters audio models to audio-capable Gemini, and uses the source-specific detail for individual and batch generation. Workers continue to advertise no generation methods. Custom Node entry points should register both available factories with the existing service and worker. No Google API key or additional dependency is required.

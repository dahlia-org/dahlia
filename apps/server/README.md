...
Apply the new PostgreSQL/SQLite/D1 `audio_summary` migration before updating Server and Web. Desktop needs no update for this feature; existing Desktop start requests follow the shared account source setting, but no audio controls are added there. Workers continue to advertise no generation methods. Custom Node entry points should register both available factories with the existing service and worker. No Google API key or additional dependency is required.

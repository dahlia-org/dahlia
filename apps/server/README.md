...
**Client compatibility:** the default Vault listing returns owned Vaults. Clients needing every accessible Vault use `GET /api/v1/vaults?scope=accessible`, which includes owned Vaults and current direct-user, Organization, and Team member grants. This scope cannot be combined with `userId` or `organizationId` (`400`). It reuses the normal read authorization and returns each Vault once. Desktop discovery uses this scope; update Server before or together with Desktop. Older Servers do not support the scope, while existing registered Vaults retain their normal sync path. `GET /api/v1/organizations` lists the authenticated user's Organizations in both accounts and header modes and accepts browser or `all-apis` gateway authentication.


Files API storage currently requires the Databricks Volume backend. Reserve a client-generated UUIDv7 and attributes using JSON, then stream immutable bytes by PUT. `name` is 1–255 characters, `metadata.source` is `upload` or `screenshot`, and optional dimensions are positive integers up to 33,554,432. OCR and captions belong in later metadata mutations. PUT requires Content-Length up to 64 MiB and application/octet-stream; the MIME type comes from the reservation. Server computes size and SHA-256 while streaming. A missing length returns 411, an excessive length 413, and unsupported encoding 415.

```http
POST /api/v1/file-uploads
Content-Type: application/json

{"id":"<UUIDv7>","vaultId":"<UUID>","name":"capture.png","contentType":"image/png","metadata":{"source":"screenshot","width":1800,"height":900}}
```
...
- `recording:upsert` in `/api/v1/transactions`: `entityId` is the internal session UUID, data contains `source`, `checksum`, and `manifest` (`sampleRate: 16000`, `frameCount`, and ranges with `startFrame`, `frameCount`, `sessionOffsetSeconds`, `localeIdentifier`). PUT staging is private to the owner until this commit.

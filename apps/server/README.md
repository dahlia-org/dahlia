...
| `/sign-in` | Google account sign-in | Redirect to `/dashboard`; the accounts sign-in screen is unavailable |
| `/oauth/consent` | Dahlia Desktop OAuth consent | Redirect to `/dashboard`; the accounts consent screen is unavailable |
...
`DAHLIA_SIGNOUT_URL` sets the browser destination after Dahlia clears its local session. It accepts a root-relative path or an HTTPS URL (HTTP is allowed only for localhost). The default is `/sign-in` for `accounts` authentication and `/dashboard` for Header authentication. Set it to the authenticating proxy's logout endpoint when signing out must also end the upstream session; Databricks Apps uses `/.auth/logout`.

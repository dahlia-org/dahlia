...
The Private Web sidebar displays collapsible Projects and their meetings, with dates and the current meeting highlighted. Hovering or keyboard-focusing a meeting shows a detail panel to its right with the title, duration (or recording status), Project, date, and description; Escape dismisses it. Project rows use the same hover/focus panel to show their name, meeting count, and description. Only the selected Workspace’s Projects and meetings appear in the sidebar, without a Workspace-name heading; switch Workspaces from the account menu. Meetings without a Project remain accessible under **Unassigned**. **New Workspace** remains available while an Organization is selected. Creation requires a Team Organization and opens the new Workspace in that Organization; Workspace Admins can then share it from its detail page. Personal Organizations cannot contain additional Workspaces. Workspace routes determine the selection; other pages restore the last selection per user and Personal/Organization scope from tab-scoped session storage, defaulting to the first available Workspace. Expand controls are separate from detail links; meetings load on expansion, with **Show more** for subsequent pages. The sidebar footer shows the account name. Its compact account menu uses icons and separate Workspace and Organization sections; the current Workspace has a checkmark and each Workspace links to its details. The account row opens account information. The remaining entries provide Personal/Organization switching, Workspace and Organization management, account settings and capability-gated extension links. **Members** appears inside the **Organizations** section for platform administrators, including deployments without sharing. Both auth modes clear the Dahlia browser session from the menu and then use the configured sign-out destination; sign-out errors stay visible for retry. Header deployments can route that destination through the upstream proxy's logout endpoint. Selection and expansion are stored per user in tab-scoped session storage. The initial selection is Personal; a revoked Organization selection returns to Personal.
...
Both auth modes use Better Auth Web sessions, Organizations, invitations, and Teams. Header requests still require verified proxy identity and reject a mismatched session user. Every new user receives a Personal Organization and one private Personal Workspace. Trusted Header and verified Google users join all matching `auto_join` Organizations as members on first registration. Native administrator user creation in Header mode atomically provisions the same Header identity and Organizations, so subsequent proxy sign-in retains the created user and its grants. Header user emails cannot be edited through Better Auth: a changed proxy email represents a separate identity. Only Server administrators can create or delete Team Organizations; creation requires an explicit initial owner and does not enroll the actor automatically. Departure or removal is not undone at sign-in, but users may explicitly rejoin under the current domain policy. Deletion requires removing owned Workspaces first and preserving every shared Workspace's last effective Admin; Personal sharing/deletion is prohibited. Organization/team membership and permission changes are validated atomically, including concurrent attempts to remove the last effective Admin. Account deletion is disabled.
...
Resolving the initial Personal Organization in Private Web preserves unsaved form input; selecting another Organization resets the previous scope’s form state.
...
`pnpm dev` / `pnpm dev:api` enables sample data for local SQLite when `DAHLIA_APP_URL` uses localhost or a loopback address. After normal sign-in, a Personal Workspace without synchronization history receives a Project and three meetings with summaries and transcripts, including an unclassified meeting. Existing sample meetings from older seeds receive their missing transcripts on the next sign-in; other content and history are left untouched. Repeated requests and restarts do not duplicate the sample. The normal transaction service handles serialization and search indexing. PostgreSQL, Lakebase, Workers, and `pnpm start` do not seed data. Sample content can be removed through ordinary content actions; Personal Workspace itself cannot be deleted. Google sign-in configuration is required only in accounts mode.
...
The first registered user becomes the Server administrator. In both auth modes, creation initializes Personal, its owner membership, Personal Workspace and direct Workspace Admin permission. Trusted Header and verified Google registrations also join every matching `auto_join` Team Organization as a member. Existing users are never enrolled solely by sign-in or a settings change. Leaving or removal does not ban future participation: users may explicitly join or apply under the current policy. Organization membership grants no Workspace access.
...
Automatically created Personal Organization slugs use the email local part. They lowercase the source and replace each non-ASCII-alphanumeric character with `_` (for example, `kazuki.matsuda` becomes `kazuki_matsuda`). An empty source in local single-user mode uses `organization`. Collisions receive `_2`, `_3`, etc. inside the authorization transaction, using one lookup of occupied slugs and choosing the first vacant suffix in memory. Existing slugs are unchanged. Organization Settings pairs each name or slug value with its edit action in a responsive row and separates leaving from general settings; deletion belongs to Server administration. The Personal owner or Team owner/admin can change the slug through the existing organization update API. Manual slugs accept lowercase ASCII letters, digits, `_`, and `-`; empty, invalid, duplicate, and Team `personal-` reserved slugs are rejected. Organization URLs use `/orgs/{organizationId}` and remain unchanged when the slug is edited. IDs, domains, and membership remain unchanged.
...
`/admin/orgs` lists Team Organizations, including those the administrator has not joined; `/admin/users` lists registered users and the existing administrator controls.

...
To regenerate the catalog from an OpenAI Codex checkout at `rust-v0.153.4`, run `node scripts/generate-databricks-models.mjs /path/to/codex/codex-rs/models-manager` from `apps/server`. The generator copies selected runtime fields, expands both ID spellings, assigns display names, and adds non-OpenAI models. Non-OpenAI Flash entries use Luna runtime parameters; GLM 5.3, Kimi K3, and DeepSeek V4 Pro use Sol parameters, with their own names, reasoning settings, descriptions, and input modalities. GPT identity is removed from their instruction templates. These are Dahlia runtime defaults, not claims of provider feature parity. It also updates the separate fallback prompt. Description sources (checked 2026-09-08): [GLM 5.3](https://huggingface.co/zai-org/GLM-5.3), [GLM 5.3 Flash](https://huggingface.co/zai-org/GLM-5.3-Flash), [Kimi K3](https://huggingface.co/moonshotai/Kimi-K3), [DeepSeek V4 Pro](https://www.deepseek.com/), [Gemini 3.8 Flash](https://ai.google.dev/gemini-api/docs/latest-model), and [Gemini 3.7 Flash](https://blog.google/innovation-and-ai/models-and-research/gemini-models/introducing-gemini-3-7-flash/). DeepSeek `-0813` uses the same family description. Codex source files are only used during generation; Server startup reads the committed catalog. Review generated changes and update the helper/version contract when advancing the referenced Codex release.

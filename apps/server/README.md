...
Future user banks and a memory router are intentionally absent. Keep thread ownership, Workspace access and bank destination separate. A later router should propose typed candidates (`workspace` evidence/explicit shared information, `user` preferences/interactions), apply permission and sharing policy server-side, and abstain on ambiguity. The model must never supply an arbitrary bank ID or copy private chat/personal preferences into a shared bank automatically; Workspace-derived knowledge must retain its Workspace authorization and source lineage.

Run `pnpm test:memory` for focused memory checks. To include forced-RLS/shared-pool coverage, set `TEST_DATABASE_URL` to a dedicated migrated PostgreSQL test database owned by a non-superuser role without `BYPASSRLS`; never point tests at a real user's database. The package ships all registered SQL migrations through `drizzle/*/*/migration.sql`, including the new memory tables and FORCE RLS migration.

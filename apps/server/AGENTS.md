...
- Organizations own Workspaces. Workspace admin manages sharing and the Workspace; admin/editor can mutate content and viewer can read. Organization membership alone grants no Workspace access. Server meeting MCP remains read-only. Dahlia Memory mutations require their dedicated scope and current owner/Workspace authorization; follow docs/adr/server/dahlia-memory.md. Follow docs/adr/shared/organization-vaults.md.
...
- Better Auth and Dahlia application tables share one Drizzle application database. All authentication modes use the generated `auth` tables and `app` for canonical data, `search` for search projections, `crypto` for wrapped keys, `jobs` for job tables, and the independently migrated `agent` schema for AI history. Header mode projects validated users into `auth.user` without starting the Better Auth runtime. References flow from `jobs` to `app` / `auth` and from `app` to `auth`. Node supports SQLite, PostgreSQL, and Lakebase; Workers support Hyperdrive, and direct PostgreSQL.
- Released migrations are immutable. Server is currently unreleased: its migrations may be directly edited and consolidated with matching Drizzle snapshots and manifest updates. This does not authorize deleting existing databases or rewriting their migration ledgers; use a fresh disposable database for validation. Desktop released migrations remain immutable.

...
describe.runIf(databaseUrl)("PostgreSQL targeted summary delivery", () => {
  it.each(["duplicate", "retry", "lease", "cancel", "permission", "conflict"])("protects durable results across %s delivery", async (scenario) => {
    ...
    const userId = uuidV7(); const identity = { userId,  source: "header" as const };
    const sync = new MeetingSyncService(store.sync);
    const workspaceId = uuidV7(), meetingId = uuidV7(); const now = new Date().toISOString();
    ...
    const method: SummaryMethod = { id: "transcript", captureSettings: (settings) => ({
      ...
      version: () => Promise.resolve("v1"), generate: async () => {
        generations++;
        if (scenario === "permission") await store.sync.withIdentity(identity, (scoped) => scoped.deletePermission(workspaceId, "user", userId));
    ...
    try {
      ...
      await sync.commitTransaction(identity, { schemaVersion: 3, id: uuidV7(), workspaceId, createdAt: now, operations: [
        { id: uuidV7(), entity: "workspace", action: "create", entityId: workspaceId, baseRevision: null, data: { organizationId: testOrganizationID, name: "Synthetic", createdAt: now } },
      ...
      if (scenario === "cancel") {
        await service.cancel(identity, workspaceId, meetingId, accepted.id);
      ...
      expect(await jobs.claim(reference)).toBeNull();
      if (scenario !== "permission") {
        const status = await service.status(identity, workspaceId, meetingId);
        ...
        const versions = await store.sync.withIdentity(identity, (scoped) => scoped.listSummaryVersions(workspaceId, meetingId, 100));

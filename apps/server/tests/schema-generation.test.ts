...
describe("auth schema generation", () => {
  it("reproduces the committed artifact-aware schemas", () => {
    ...
    try {
      ...
      for (const name of ["auth-schema.ts", "sqlite-schema.ts"]) {
        expect(readFileSync(join(directory, "src/db", name), "utf8"))
          .toBe(readFileSync(new URL(`../src/db/${name}`, import.meta.url), "utf8"));
      }
    } finally {

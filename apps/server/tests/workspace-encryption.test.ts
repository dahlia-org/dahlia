...
import { createWorkspaceCipher, encodeBase64, encryptionConfig, unwrapDataKey, wrapDataKey } from "../src/encryption/crypto";
...
it("authenticates ciphertext identity and rewraps without changing the data key", async () => {
  ...
  const wrapped = await wrapDataKey(config1, "workspace", raw);
  expect(await unwrapDataKey(config3, "workspace", wrapped)).toEqual(raw);
  await expect(unwrapDataKey(config1, "other-workspace", wrapped)).rejects.toThrow("workspace_encryption_unavailable");
  const cipher = await createWorkspaceCipher("workspace", raw);
  ...
  expect(await unwrapDataKey(config3, "workspace", await wrapDataKey(config3, "workspace", raw))).toEqual(raw);
...
async function setup(mode: "none" | "server" = "server") {
  ...
  const transaction = (operations: SyncTransaction["operations"], workspace = workspaceId): SyncTransaction => ({
    schemaVersion: 3, id: uuidV7(), workspaceId: workspace, createdAt: new Date(), requestHash: uuidV7(), operations,
  });
  const commit = (tx: SyncTransaction) => store.sync.withIdentity(owner, (sync) => sync.commitTransaction(tx));
  await commit(transaction([{ id: uuidV7(), entity: "workspace", action: "create", entityId: workspaceId, baseRevision: null,
    data: { organizationId: testOrganizationID, name: "PRIVATE_WORKSPACE_MARKER", encryption: mode, createdAt: new Date() } }]));
  const db = new DatabaseSync(path);
  cleanups.push(async () => db.close());
  return { db, path, directory, workspaceId, transaction, commit, config,
...
it("encrypts canonical content, patches and receipts while preserving plaintext search data", async () => {
  ...
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.putTranscriptChunk(f.workspaceId, meetingId, patchId, 0, hash,
  ...
  const read = () => f.store.sync.withIdentity(owner, (sync) => sync.listTranscript(f.workspaceId, meetingId, 10));
  expect(await read()).toMatchObject([{ text: "PRIVATE_TRANSCRIPT_MARKER", speakerLabel: "PRIVATE_SPEAKER_MARKER" }]);
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.getMeeting(f.workspaceId, meetingId))).toMatchObject({
  ...
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.listMeetings(f.workspaceId,
  ...
  for (const table of ["workspaces", "meetings", "summaries", "transcripts", "transcript_segments", "transaction_receipts"]) {
  ...
  expect(await read()).toMatchObject([{ text: "PRIVATE_TRANSCRIPT_MARKER", speakerLabel: "PRIVATE_SPEAKER_MARKER" }]);
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.listTranscript(f.workspaceId, meetingId, 10, undefined, 1)))
  ...
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.listTranscriptAnalytics(f.workspaceId, meetingId, 2)))
  ...
  await expect(read()).rejects.toThrow("workspace_encryption_unavailable");
...
it("preserves protected file metadata and job input across partial updates and claims", async () => {
  ...
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.getScreenshot(f.workspaceId, meetingId, fileId)))
  ...
  const cancelled = await f.store.sync.withIdentity(owner, (sync) => sync.cancelSummaryJob(f.workspaceId, meetingId, job.id));
...
it("rejects mode changes and transfers, rolls back failed writes, and compacts encrypted receipts", async () => {
  const f = await setup();
  await expect(f.commit(f.transaction([{ id: uuidV7(), entity: "workspace", action: "update", entityId: f.workspaceId, baseRevision: 1,
    data: { name: "changed", encryption: "none" } }]))).rejects.toMatchObject({ code: "workspace_encryption_immutable" });
  await f.commit(f.transaction([{ id: uuidV7(), entity: "workspace", action: "update", entityId: f.workspaceId, baseRevision: 1,
    data: { name: "PRIVATE_UPDATED_MARKER" } }]));
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.getWorkspace(f.workspaceId))).toMatchObject({ encryption: "server", name: "PRIVATE_UPDATED_MARKER" });
  ...
  await expect(f.commit(f.transaction([
    ...
    { id: uuidV7(), entity: "workspace", action: "update", entityId: f.workspaceId, baseRevision: 999, data: { name: "stale" } },
  ]))).rejects.toMatchObject({ code: "revision_conflict" });
  expect(f.db.prepare("SELECT count(*) AS n FROM projects").get()!.n).toBe(before);
  const destination = uuidV7();
  await f.commit(f.transaction([{ id: uuidV7(), entity: "workspace", action: "create", entityId: destination, baseRevision: null,
    data: { organizationId: testOrganizationID, name: "Normal", createdAt: new Date() } }], destination));
  await expect(f.store.sync.withIdentity(owner, async (sync) => sync.transferWorkspace({ sourceWorkspaceId: f.workspaceId, destinationWorkspaceId: destination,
    sourceRevision: 2, destinationRevision: 1, idempotencyKey: uuidV7(), requestHash: uuidV7(),
    audienceHash: (await sync.workspaceTransferAudience(f.workspaceId, destination)).audienceHash }))).rejects.toMatchObject({ code: "encrypted_workspace_transfer_unsupported" });
  f.db.exec("UPDATE transaction_receipts SET created_at = 0");
  expect(await f.store.sync.pruneHistoryBatch({ workspaceId: f.workspaceId })).toMatchObject({ receiptsCompacted: 2 });
  const wrapped = f.db.prepare("SELECT wrapped_key FROM workspace_keys WHERE workspace_id = ?").get(f.workspaceId)!.wrapped_key as string;
  const cipher = await createWorkspaceCipher(f.workspaceId, await unwrapDataKey(config1, f.workspaceId, wrapped));
  const receipts = f.db.prepare("SELECT transaction_id, encrypted_payload FROM transaction_receipts WHERE workspace_id = ?").all(f.workspaceId);
...
it("restores a database backup only with its matching master key", async () => {
  ...
  expect(await restored.sync.withIdentity(owner, (sync) => sync.getWorkspace(f.workspaceId)))
    .toMatchObject({ name: "PRIVATE_WORKSPACE_MARKER" });
  ...
  await expect(wrong.sync.withIdentity(owner, (sync) => sync.getWorkspace(f.workspaceId))).rejects.toThrow("workspace_encryption_unavailable");
});
...
it.each(["none", "server"] as const)("avoids per-segment preservation lookups for full transcript writes (%s)", async (mode) => {
  ...
  await f.store.sync.withIdentity(owner, (sync) => sync.putTranscriptChunk(f.workspaceId, meetingId, patchId, 0, hash, segments, []));
  ...
  try {
    ...
    await f.store.sync.withIdentity(owner, (sync) => sync.putTranscriptChunk(f.workspaceId, meetingId, updatePatch, 0, hash, [updated], []));
    prepare.mockClear();
    ...
    const read = (version: number) => f.store.sync.withIdentity(owner, (sync) => sync.listTranscript(f.workspaceId, meetingId, 600, undefined, version));
...
it.each(["none", "server"] as const)("preserves file and job ID conflicts across Workspaces (%s source)", async (mode) => {
  ...
  await f.commit(f.transaction([
    { id: uuidV7(), entity: "workspace", action: "create", entityId: destination, baseRevision: null,
  ...
  const file = { fileId: uuidV7(), workspaceId: f.workspaceId, name: "original.png", uri: "private", offset: 0, size: 0, checksum: "", contentType: "image/png",
  ...
  await f.store.sync.withIdentity(owner, (sync) => sync.insertSummaryJob(job));
  await expect(f.store.sync.withIdentity(owner, (sync) => sync.reserveFile({ ...file, workspaceId: destination }))).resolves.toBeNull();
  await expect(f.store.sync.withIdentity(owner, (sync) => sync.insertSummaryJob({ ...job, workspaceId: destination, meetingId: targetMeetingId })))
    .rejects.toMatchObject({ code: "summary_id_reused", status: 409 });
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.getFile(file.fileId))).toMatchObject({ workspaceId: f.workspaceId, name: file.name });
  expect(await f.store.sync.withIdentity(owner, (sync) => sync.getSummaryJob(f.workspaceId, meetingId, job.id)))
    .toMatchObject({ workspaceId: f.workspaceId, requestHash: "source", input: job.input });
});

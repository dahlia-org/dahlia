...
describe("browser mutation origin", () => {
  it("allows reads and same-origin writes", () => {
    expect(mutationOriginAllowed(new Request("https://dahlia.example/api/v1/sessions"), "https://dahlia.example")).toBe(true);
    expect(
      mutationOriginAllowed(
        new Request("https://dahlia.example/api/v1/sessions", {
          method: "POST",
  ...
  it("rejects missing and cross-origin mutation origins", () => {
    expect(
      mutationOriginAllowed(
        new Request("https://dahlia.example/api/v1/sessions", { method: "POST" }),
        "https://dahlia.example",
      ),
    ).toBe(false);
    expect(

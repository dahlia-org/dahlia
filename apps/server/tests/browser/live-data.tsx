...
const workspace = { workspaceId: "v1", organizationId: "o1", organizationName: previewMode ? (ja ? "ダリア製品チーム" : "Dahlia Product Team") : "Test Organization", name: previewMode ? (ja ? "プロダクト開発" : "Product & design") : "Test Workspace", role: "owner", hasResources: true, revision: 1, createdAt: "2026-09-07T00:00:00Z" };
...
async function run() {
  ...
  const accountMenuTrigger = document.querySelector<HTMLButtonElement>('[popoverTarget="account-menu"]')!;
  accountMenuTrigger.click();
  button("Connect with MCP").click();
  await until(() => document.querySelector(".mcp-connection-output")?.textContent?.includes("uc-mcp-proxy"));
  assert(document.querySelector(".mcp-proxy-note")?.textContent?.includes("Databricks Apps authentication"), "Databricks Apps guidance is missing");
  assert(document.querySelector('.mcp-client-tabs[role="group"] button[aria-pressed="true"]')?.textContent === "mcp.json", "MCP client selection must use native toggle semantics");
  const profile = document.querySelector<HTMLInputElement>(".mcp-profile-field input")!;
  ...
  button("Claude Code").click();
  await until(() => document.querySelector(".mcp-connection-output")?.textContent?.includes("--profile 'engineering'"));
  Object.defineProperty(navigator, "clipboard", { value: { writeText: () => Promise.reject(new Error("denied")) }, configurable: true });
  button("Copy").click();
  await until(() => document.querySelector(".mcp-connection-dialog [role=alert]")?.textContent?.includes("copy them manually"));
  assert(document.querySelector(".mcp-connection-output"), "Clipboard failure must keep settings available for manual copy");
  Reflect.deleteProperty(navigator, "clipboard");
  button("Done").click();
  await until(() => !document.querySelector(".mcp-connection-dialog"));
  assert(document.activeElement === accountMenuTrigger, "Closing MCP settings must restore focus to the account menu");

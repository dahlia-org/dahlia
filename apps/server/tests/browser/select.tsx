...
function Fixture() {
  ...
  return <main style={{ padding: 16 }}><label>Choice<Select value={value} onValueChange={setValue}>
  ...
  <Select aria-label="Searchable" value="target" search={{ value: search, onValueChange: setSearch, placeholder: "Search" }} onValueChange={() => {}}><option value="target">Target</option></Select></main>;
}
...
async function run() {
  ...
  while (!document.querySelector("fieldset button")) await frame();
  ...
  key("ArrowDown"); await frame(); assert(document.activeElement?.textContent === "Beta", "disabled option not skipped");
  (document.activeElement as HTMLButtonElement).click(); await frame();
  assert(trigger.dataset.value === "b", "selection not committed");
  trigger.click(); await frame(); await frame(); key("Home"); await frame(); key("Escape"); await frame();
  assert(trigger.dataset.value === "b" && document.activeElement === trigger, "Escape changed selection or lost focus");
  trigger.click(); await frame(); await frame(); key("a"); await frame();
  ...
  assert(popup.textContent?.includes("Workspaces"), "menu label missing");
  ...
  key("Tab"); await frame();
  assert(searchable.ariaExpanded === "false" && document.activeElement === searchable, "Tab left the searchable picker open or lost trigger focus");
  document.getElementById("result")!.textContent = "PASS: disabled fieldset, selection, arrow keys, disabled option, Home, Escape, Tab, focus, typeahead";
}

...
window.fetch = async (input, init) => {
  ...
  if (url.pathname === "/api/v1/organizations" && method === "POST") {
    ...
    const organization = { id: "org_00000000000000000000000002", name: body.name, slug: body.slug };
...
const openPicker = (trigger: HTMLElement) => trigger.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", bubbles: true, cancelable: true }));
const main = () => document.querySelector("main")!;
const panel = () => document.querySelector<HTMLElement>('[role="tabpanel"]:not([hidden])')!;
function fill(name: string, value: string) {
  const input = document.querySelector<HTMLInputElement>(`[role="dialog"] input[name="${name}"]`)!;
...
async function run() {
  ...
  const accountMenu = document.querySelector<HTMLButtonElement>('[aria-label^="Account menu"]')!;
  accountMenu.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, button: 0 }));
  await until(() => document.querySelector('[data-slot="dropdown-menu-content"] a[href="/orgs"]'));
  assert(document.querySelector('[data-slot="dropdown-menu-content"] a[href="/orgs"][aria-current="page"]'), "Organization list is not selected");
  accountMenu.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, button: 0 }));
  ...
  activateTab("Workspace governance");
  ...
  await until(() => !document.querySelector(".action-dialog"));
  const workspaceRow = [...panel().querySelectorAll<HTMLElement>(".row")].find((row) => row.textContent?.includes("Team workspace"));
  assert(workspaceRow?.querySelector("svg"), "Workspace icon is missing");
  ...
  assert(button("Leave", panel()), "Leave action depends on the visible member page");
  assert(location.pathname === "/orgs/org_00000000000000000000000001", "Detail did not use TypeID");
  await until(() => [...document.querySelectorAll('[role="tab"]')].map((el) => el.textContent?.replace(/\s/g, "")).join() === "Workspacegovernance,Members1,Teams1,Settings");
  assert(![...panel().querySelectorAll("h2, h3")].some((heading) => heading.textContent === "Members"), "Member heading duplicates tab");
  ...
  assert(panel().querySelector('details[open] [role="checkbox"]'), "Team membership controls missing");
  ...
  createButton.click();
  await until(() => document.querySelector('[role="dialog"]'));
  const ownerSelect = document.querySelector<HTMLButtonElement>('[role="dialog"] [role="combobox"]')!;
  assert(ownerSelect.dataset.value === "__dahlia_empty__", "Initial owner was selected implicitly");
  openPicker(ownerSelect);
  await until(() => document.querySelector<HTMLInputElement>('[data-slot="select-content"][data-state="open"] input[type="search"]'));
  let search = document.querySelector<HTMLInputElement>('[data-slot="select-content"][data-state="open"] input[type="search"]')!;
  let ownerMenu = search.closest<HTMLElement>('[data-slot="select-content"]')!;
  ...
  search = document.querySelector<HTMLInputElement>('[data-slot="select-content"][data-state="open"] input[type="search"]')!;
  ownerMenu = search.closest<HTMLElement>('[data-slot="select-content"]')!;
  ...
  assert(ownerSelect.dataset.value === "__dahlia_empty__", "Filtered owner was selected implicitly");
  ownerMenu.querySelector<HTMLElement>('[role="option"]')!.click();
  await until(() => ownerSelect.dataset.value === "second");
  await new Promise(requestAnimationFrame);
  openPicker(ownerSelect);
  await until(() => document.querySelector('[data-slot="select-content"][data-state="open"]'));
  search = document.querySelector<HTMLInputElement>('[data-slot="select-content"][data-state="open"] input[type="search"]')!;
  ...
  assert(ownerSelect.dataset.value === "second", "Owner selection was reset by a live result refresh");
  Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")!.set!.call(search, "owner");
  search.dispatchEvent(new Event("input", { bubbles: true }));
  await until(() => ownerMenu.querySelectorAll('[role="option"]').length === 1 && ownerSelect.dataset.value === "__dahlia_empty__");
  ownerMenu.querySelector<HTMLElement>('[role="option"]')!.click();
  await until(() => ownerSelect.dataset.value === "owner");
  ...
  document.querySelector<HTMLButtonElement>('[role="dialog"] [data-confirm]')!.click();
  ...
  failCreate = false;
  document.querySelector<HTMLButtonElement>('[role="dialog"] [data-confirm]')!.click();
  await until(() => location.pathname === "/admin/orgs" && !document.querySelector('[role="dialog"]'));
  ...
  assert(!button("Delete organization", main()), "Organization deletion is detached from Settings");
  activateTab("Settings");
  ...
  assert(policy.dataset.value === "invite_only", "New domain is not invitation-only");
  ...
  openPicker(policy);
  await until(() => document.querySelector('[data-slot="select-content"][data-state="open"]'));
  document.querySelector<HTMLElement>('[data-slot="select-content"][data-state="open"] [role="option"][data-value="auto_join"]')!.click();
  await until(() => policy.dataset.value === "auto_join");
  document.querySelector<HTMLButtonElement>(".action-dialog [data-confirm]")!.click();
  await until(() => !document.querySelector(".action-dialog") && panel()?.textContent?.includes("company.example"));
  const domainRow = button("Remove", panel()).closest<HTMLElement>(".org-domain-row")!;
  assert(domainRow.textContent?.includes("Auto join"), "Domain join policy is unclear");
  ...
  const joined = organizations.splice(0);
  navigateDashboard("/orgs");
  ...
  organizations.push(...joined);
  window.dispatchEvent(new Event(clientMutationEvent));
  activeRole = "member";
  navigateDashboard("/orgs/org_00000000000000000000000001");
  await until(() => button("Settings"));
  activateTab("Settings");
  await until(() => panel()?.textContent?.includes(organizations[0]!.slug));

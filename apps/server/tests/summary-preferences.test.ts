...
it("keeps inactive overrides and rejects unavailable explicit models and effort", () => {
  ...
  preferences.processing = { location: "remote", remote: {
    workflow: "combined", summaryModel: "system.ai.gemini-3-8-flash", transcriptionModel: "unavailable",
  } };
  ...
  expect(resolve().settings.model).toBe("system.ai.gemini-3-8-flash");
...
it("resolves retranscription to Gemini without language or summary settings", () => {
  const preferences = structuredClone(DEFAULT_WORKSPACE_GENERATION_SETTINGS);
  preferences.processing = { location: "remote", remote: {
    workflow: "combined", summaryModel: "unavailable", transcriptionModel: "system.ai.gemini-3-8-flash", reasoningEffort: "max",
  } };

...
// Codex main 713caa89 (existing entries) and 694d8d45 (GPT-6 Sol/Luna), with Databricks slugs, plan availability, and display names.
const upstreamHashes: [string, string][] = [["gpt-6-astra","2398784eb6729f680770523fa868e676a342a6842433508fdeb34b87b77c112c"],["gpt-6-sol","fd36871888525ad8f4158bed9409064f43b017a1a0efe7bac3aa1c39c09178a3"],["gpt-6-luna","78228fe201f4bbdf72fdc78cd70a22e16075de9cd0ef706b4f97c627bbc3abcd"],["gpt-5-6-sol","8a0319235164a1db323baf7de68b862ad1becd2034972862a31b332dde1aed42"],["gpt-5-6-terra","40652a62267fb6dbcb38c24f04abb24f2880a2cf5a7c43e36fbbe4191eb351ac"],["gpt-5-6-luna","5db57f41f678e0166165febfa8760bbfa109ccd6e175acd483cf58523a63e065"],["gpt-5-5","841469379b687aa687dc5431e754d2a60baaa408b024bfe2573ab50c9f1bd55b"]];
...
describe("model catalog", () => {
  ...
  it("contains only the requested GPT families and fully qualified Databricks IDs", () => {
    expect(catalog.models.filter(({ slug }) => slug.startsWith("system.ai.gpt-")).map(({ slug }) => slug))
      .toEqual(upstreamHashes.map(([slug]) => `system.ai.${slug}`));
    expect(catalog.models.every(({ slug }) => slug === "codex-auto-review" || slug.startsWith("system.ai."))).toBe(true);
  ...
  it("does not normalize IDs or create definitions for unknown models", () => {
    const ids = ["gpt-5.6-sol", "gpt-5-6-sol", "GPT-5-6-SOL", " gpt-5-6-sol ", "gpt-future", "glm-future", "custom"];
  ...
  it("generates hidden built-ins without publishing them as available", () => {
    ...
    expect(list.models.find(({ slug }) => slug === "system.ai.gpt-5-5")?.visibility).toBe("list");
  });
  ...
  it.each([
    ...
    const model = catalog.models.find((model) => model.slug === `system.ai.${slug}`)!;
    const reference = catalog.models.find((model) => model.slug === `system.ai.${referenceSlug}`)!;
  ...
  it("keeps non-GPT requests on the standard Responses wire format", () => {
    ...
    for (const model of catalog.models.filter(({ slug }) => !slug.startsWith("system.ai.gpt-") && slug !== "codex-auto-review")) {
  ...
  it("uses DeepSeek's documented high default and optional non-thinking mode", () => {
    const model = catalog.models.find((model) => model.slug === "system.ai.deepseek-v4-pro-0813")!;
  ...
  it("uses Luna runtime metadata and Pro reasoning choices for DeepSeek V4.1 Flash", () => {
    const model = catalog.models.find((model) => model.slug === "system.ai.deepseek-v4-1-flash")!;
    const luna = catalog.models.find((model) => model.slug === "system.ai.gpt-5-6-luna")!;
  ...
  it.each(["gemini-3-8-flash", "gemini-3-7-flash"])("hides %s while retaining audio summary metadata", (id) => {
    const list = modelList([{ id: `system.ai.${id}` }]);
    expect(list.data[0]?.id).toBe(`system.ai.${id}`);

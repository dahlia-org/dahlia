...
const owner: Identity = { userId: testUserID("retention-owner"),  source: "header" };
const member: Identity = { userId: testUserID("retention-member"),  source: "header" };
...
function body(workspaceId: string, operations: Omit<SyncTransactionOperation, "id">[]) {
  return {
    schemaVersion: 3, id: id(), workspaceId, createdAt: new Date().toISOString(),
...
async function setup() {
  ...
  const create = body(workspaceId, [{ entity: "workspace", action: "create", entityId: workspaceId, baseRevision: null, data: { organizationId: testOrganizationID,
    name: "Durable Workspace", createdAt: new Date().toISOString(),
  } }]);
  const receipt = await service.commitTransaction(owner, create);
  return { directory, path, config, store, raw, service, workspaceId, create, receipt };
}
...
describe("sync history retention", () => {
  it.each(["deleted", "recreated-empty", "recreated-summary", "updated"])(
    "keeps child invalidations complete when a snapshotted meeting is %s", async (scenario) => {
      ...
      raw.prepare("INSERT INTO files(file_id, workspace_id, uri, size, content_type, checksum, name, metadata, active, uploaded_at, revision) VALUES (?, ?, ?, 1, 'image/png', ?, 'capture.png', ?, 1, ?, 1)")
        .run(screenshotId, workspaceId, `/Volumes/test/app/files/files/${screenshotId}/original`, `SHA-256:${"a".repeat(64)}`, '{"source":"screenshot"}', Date.now());
      raw.prepare("INSERT INTO meeting_attachments(id, workspace_id, meeting_id, file_id, captured_at) VALUES (?, ?, ?, ?, ?)")
        .run(screenshotId, workspaceId, meetingId, screenshotId, Date.now());
      const snapshot = await service.listSnapshot(owner, workspaceId);
      expect(snapshot.items.map(({ entity }) => entity)).toEqual(["workspace", "meeting", "summary", "transcript", "file", "meeting_attachment"]);
      if (scenario === "updated") {
      ...
      const delta = await service.listChanges(owner, workspaceId, snapshot.startCursor);
      if (scenario === "updated") {
  ...
  it("rolls back a failed SQLite prune and drains a ledger across bounded batches", async () => {
    const { store, raw, workspaceId, receipt } = await setup();
    const insert = raw.prepare("INSERT INTO sync_changes(workspace_id, entity, entity_id, action, revision, transaction_id) VALUES (?, 'workspace', ?, 'upsert', 1, ?)");
    raw.exec("BEGIN");
    for (let index = 0; index < 1_000; index++) insert.run(workspaceId, workspaceId, id());
    raw.prepare("UPDATE sync_workspace_state SET latest_sequence = (SELECT max(sequence) FROM sync_changes) WHERE workspace_id = ?").run(workspaceId);
    raw.exec("COMMIT");
    expire(raw);
    ...
    expect(raw.prepare("SELECT pruned_through FROM sync_workspace_state").get()).toEqual({ pruned_through: 0 });
    expect(raw.prepare("SELECT count(*) AS count FROM sync_changes").get()).toEqual({ count: 1_001 });
    raw.exec("DROP TRIGGER fail_prune");
    const first = await store.sync.pruneHistoryBatch({ workspaceId });
    ...
    expect(raw.prepare("SELECT latest_sequence = pruned_through AS complete FROM sync_workspace_state").get()).toEqual({ complete: 1 });
  ...
  it("compacts plaintext receipts in bounded SQL batches and resumes without work", async () => {
    const { store, raw, workspaceId, create } = await setup();
    const copy = raw.prepare(`INSERT INTO transaction_receipts
      (transaction_id, owner_user_id, workspace_id, request_hash, response_json, results_json, cursor, created_at)
      SELECT ?, owner_user_id, workspace_id, request_hash, response_json, results_json, cursor, created_at
    ...
    expect(await store.sync.pruneHistoryBatch({ workspaceId })).toMatchObject({ receiptsCompacted: 1_000 });
    ...
    prepare.mockClear();
    expect(await store.sync.pruneHistoryBatch({ workspaceId })).toEqual({ changesDeleted: 0, receiptsCompacted: 0 });
  ...
  it("keeps the exact 90-day boundary and compacts only older data", async () => {
    const { store, raw, workspaceId } = await setup();
    const time = Date.now();
    ...
    expect(await pruneSyncHistory(store.sync)).toEqual({ changesDeleted: 0, receiptsCompacted: 0 });
    expect(raw.prepare("SELECT name FROM workspaces WHERE workspace_id = ?").get(workspaceId)).toEqual({ name: "Durable Workspace" });
  });

  it("preserves cursors after deleting every change and bootstraps an old Workspace from canonical rows", async () => {
    const { store, service, raw, workspaceId, receipt } = await setup();
    expire(raw);
    await pruneSyncHistory(store.sync);
    expect(await service.latestCursor(owner)).toBe(receipt.cursor);
    await expect(service.listChanges(owner, workspaceId)).rejects.toMatchObject({ status: 410, code: "sync_cursor_expired" });
    expect(await service.listChanges(owner, workspaceId, receipt.cursor)).toMatchObject({ items: [], cursor: receipt.cursor, hasMore: false });
    const snapshot = await service.listSnapshot(owner, workspaceId);
    expect(snapshot).toMatchObject({ startCursor: receipt.cursor, nextCursor: null, items: [{ entity: "workspace", id: workspaceId }] });
    const next = await service.commitTransaction(owner, body(workspaceId, [{
      entity: "workspace", action: "update", entityId: workspaceId, baseRevision: 1, data: { name: "New name" },
    }]));
    expect(decodeSyncCursor(next.cursor)).toBeGreaterThan(decodeSyncCursor(receipt.cursor));
    expect(await service.listChanges(owner, workspaceId, snapshot.startCursor)).toMatchObject({ items: [{ record: { name: "New name" } }] });
  });

  it("resolves compact receipts without replay or stale content, and rejects altered requests", async () => {
    const { store, service, raw, workspaceId, create, receipt } = await setup();
    ...
    expect(compact).toEqual({
      ...
      records: [{ entity: "workspace", id: workspaceId, revision: 1 }],
    });
    expect(raw.prepare("SELECT response_json, results_json FROM transaction_receipts").get()).toEqual({
      response_json: null, results_json: JSON.stringify([{ entity: "workspace", id: workspaceId, revision: 1 }]),
    });
    ...
    expect(raw.prepare("SELECT revision FROM workspaces").get()).toEqual({ revision: 1 });
  });

  it("preserves deletion acknowledgements after the Workspace and its change history disappear", async () => {
    const { store, service, raw, workspaceId } = await setup();
    const deletion = body(workspaceId, [{ entity: "workspace", action: "reset", entityId: workspaceId, baseRevision: 1, data: {} }]);
    ...
    expect(await service.resolveTransaction(owner, deletion)).toMatchObject({
      status: "committed", receipt: "compact", cursor: receipt.cursor, records: [{ entity: "workspace", revision: null }],
    });
    expect(await service.latestCursor(owner)).toBe(receipt.cursor);
    await expect(service.listSnapshot(owner, workspaceId)).rejects.toMatchObject({ status: 404 });
    await expect(service.listChanges(member, workspaceId)).rejects.toMatchObject({ status: 404 });
  });
  ...
  it("bounds serialized snapshot bytes and resumes without skipping large summaries", async () => {
    ...
    const createdAt = new Date().toISOString();
    await service.commitTransaction(owner, body(workspaceId, meetings.map((meetingId) => ({
    ...
    expect((await service.listSnapshot(owner, workspaceId)).items
    ...
    const seen: string[] = [];
    let page = await service.listSnapshot(owner, workspaceId);
    ...
    while (true) {
      ...
      page = await service.listSnapshot(owner, workspaceId, page.nextCursor, startCursor);
    }
  ...
  it("finishes a paged snapshot with delta catch-up and refuses a pruned starting cursor", async () => {
    ...
    await service.commitTransaction(owner, body(workspaceId, projects.map((projectId) => ({
    ...
    const first = await service.listSnapshot(owner, workspaceId);
    ...
    const second = await service.listSnapshot(owner, workspaceId, first.nextCursor!, first.startCursor);
    expect(second.startCursor).toBe(first.startCursor);
    const delta = await service.listChanges(owner, workspaceId, first.startCursor);
    expect(delta.items).toEqual(expect.arrayContaining([
    ...
    expire(raw);
    await pruneSyncHistory(store.sync);
    await expect(service.listSnapshot(owner, workspaceId, first.nextCursor!, first.startCursor))
      .rejects.toMatchObject({ status: 410, code: "sync_cursor_expired" });
    await expect(service.listChanges(owner, workspaceId, first.startCursor, delta.highWaterCursor))
      .rejects.toMatchObject({ status: 410, code: "sync_cursor_expired" });
  });

  it("reauthorizes each snapshot page after sharing is revoked", async () => {
    const { store, service, raw, workspaceId } = await setup();
    raw.prepare("INSERT INTO workspace_permissions(workspace_id, principal_type, principal_id, role, granted_by_user_id) VALUES (?, 'user', ?, 'viewer', ?)")
      .run(workspaceId, member.userId, owner.userId);
    const snapshot = await service.listSnapshot(member, workspaceId);
    expect(snapshot.items).toHaveLength(1);
    raw.prepare("DELETE FROM workspace_permissions WHERE principal_id = ?").run(member.userId);
    await expect(service.listSnapshot(member, workspaceId, undefined, snapshot.startCursor)).rejects.toMatchObject({ status: 404 });
    expect(await store.sync.withIdentity(owner, (sync) => sync.getWorkspace(workspaceId))).not.toBeNull();
  });

  it("registers authenticated, body-limited resolve and snapshot routes", async () => {
    const { store, config, workspaceId, create, receipt } = await setup();
    ...
    expect((await app.request(`/api/v1/workspaces/${workspaceId}/snapshot`, { headers })).status).toBe(200);
    expect((await app.request(`/api/v1/workspaces/${workspaceId}/snapshot`)).status).toBe(401);
    ...
    expect((await app.request(`/api/v1/workspaces/${workspaceId}/snapshot?cursor=invalid`, { headers })).status).toBe(400);
  });
...
describe("partial text content", () => {
  ...
  it("synchronizes 10000 historical meetings without transferring any text body", async () => {
    ...
    const insert = raw.prepare(`INSERT INTO meetings(meeting_id, workspace_id, name, status, created_at, updated_at,
    ...
    for (let index = 0; index < 10000; index += 1) {
      const meeting = id();
      insert.run(meeting, workspaceId, index, index);
    ...
    let transcripts = 0;
    do {
      const page = await service.listSnapshot(owner, workspaceId, cursor, start);
  ...
  it("pins every content page to its revision and reauthorizes deleted or revoked content", async () => {
    ...
    await service.commitTransaction(owner, body(workspaceId, [{ entity: "meeting", action: "create", entityId: meetingId, baseRevision: null,
    ...
    raw.exec("COMMIT");
    const manifest = await service.transcriptContent(owner, workspaceId, meetingId, "1", "1");
    ...
    const first = await service.transcriptContent(owner, workspaceId, meetingId, "1");
    expect(first.items).toHaveLength(500);
    expect(first.nextCursor).toBeTruthy();
    expect((await service.transcriptContent(owner, workspaceId, meetingId, "1", undefined, first.nextCursor!)).items).toHaveLength(1);
    raw.prepare("UPDATE meetings SET transcript_revision = 2 WHERE meeting_id = ?").run(meetingId);
    expect((await service.transcriptContent(owner, workspaceId, meetingId, "1", undefined, first.nextCursor!)).items).toHaveLength(1);
    const metadata = await service.listChanges(owner, workspaceId, undefined, undefined);
    expect(metadata.items.find((item) => item.entity === "summary")?.record).toMatchObject({ contentOmitted: true, contentPresent: true });
    expect(await service.getMeeting(owner, workspaceId, meetingId)).toHaveProperty("summaryDocument", "{}");
    await expect(service.transcriptContent(member, workspaceId, meetingId, "2")).rejects.toMatchObject({ status: 404 });
    raw.prepare("UPDATE meetings SET active = 0 WHERE meeting_id = ?").run(meetingId);
    await expect(service.transcriptContent(owner, workspaceId, meetingId, "2")).rejects.toMatchObject({ status: 404 });
  });

  it("keeps search pages stable when another Workspace changes corpus-wide FTS statistics", async () => {
    const { raw, service, workspaceId } = await setup();
    const otherWorkspaceId = id();
    await service.commitTransaction(owner, body(otherWorkspaceId, [{ entity: "workspace", action: "create", entityId: otherWorkspaceId,
      baseRevision: null, data: { organizationId: testOrganizationID, name: "Other", createdAt: new Date().toISOString() } }]));
    const meeting = raw.prepare("INSERT INTO meetings(meeting_id, workspace_id, name, status, created_at, updated_at, active) VALUES (?, ?, 'Metadata', 'READY', 0, 0, 1)");
    const document = raw.prepare("INSERT INTO search_documents(document_id, workspace_id, meeting_id, kind, summary_text, search_text, embedding_content_hash) VALUES (?, ?, ?, 'meeting', ?, ?, 'hash')");
    const ids = [id(), id()];
    for (const [index, text] of ["alpha beta", "alpha beta beta beta " + "noise ".repeat(10)].entries()) {
      const meetingId = ids[index]!;
      meeting.run(meetingId, workspaceId);
      document.run(meetingId, workspaceId, meetingId, text, text);
    }
    const first = await service.searchText(owner, workspaceId, "alpha beta", "meeting", undefined, "1");
    expect(first.nextCursor).toBeTruthy();
    for (let index = 0; index < 10; index += 1) {
      const meetingId = id();
      const text = "alpha ".repeat(50) + "noise ".repeat(300);
      meeting.run(meetingId, otherWorkspaceId);
      document.run(meetingId, otherWorkspaceId, meetingId, text, text);
    }
    const second = await service.searchText(owner, workspaceId, "alpha beta", "meeting", first.nextCursor!, "1");
  ...
  it("pages every FTS result without a hybrid candidate cap and rejects malformed cursors", async () => {
    const { raw, service, workspaceId } = await setup();
    const meeting = raw.prepare("INSERT INTO meetings(meeting_id, workspace_id, name, status, created_at, updated_at, active) VALUES (?, ?, 'Metadata', 'READY', 0, 0, 1)");
    const document = raw.prepare("INSERT INTO search_documents(document_id, workspace_id, meeting_id, kind, summary_text, search_text, embedding_content_hash) VALUES (?, ?, ?, 'meeting', 'needle', ?, 'hash')");
    raw.exec("BEGIN");
    for (let index = 0; index < 1101; index += 1) {
      const meetingId = id();
      meeting.run(meetingId, workspaceId);
      document.run(meetingId, workspaceId, meetingId, "needle ".repeat(100));
    }
    raw.exec("COMMIT");
    const found = new Set<string>();
    let cursor: string | undefined;
    do {
      const page = await service.searchText(owner, workspaceId, "needle", "meeting", cursor);
    ...
    await expect(service.searchText(owner, workspaceId, "needle", "meeting", "{")).rejects.toMatchObject({ status: 400 });
    await expect(service.searchText(member, workspaceId, "needle", "meeting")).rejects.toMatchObject({ status: 404 });
  });
});

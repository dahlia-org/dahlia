...
${Object.values(contracts).map(({ operationId, method, path }) => `  ${operationId}: (init: FetchOptions<operations["${operationId}"]>, notifyMutation = ${!["get", "head"].includes(method) && !["search", "textSearch", "resolveTransaction", "personalMemoryRecall", "personalMemoryReflect", "workspaceMemoryRecall", "workspaceMemoryReflect"].includes(operationId)}) => unwrap(serverClient.${method.toUpperCase()}("${path}", init), notifyMutation),`).join("\n")}
};

...
try {
  ...
  await writeFile(join(directory, "verify.mjs"), `
    ...
    if (serverMigrationManifest.sqlite.files.length !== 16) {
    ...
    database.close();
    await store.close?.();
    if (applied.length !== serverMigrationManifest.sqlite.files.length || applied.at(-1)?.name !== "20260908093035_stale_sue_storm") {

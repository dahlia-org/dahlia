...
try {
  ...
  await writeFile(join(directory, "verify.mjs"), `
    ...
    if (serverMigrationManifest.sqlite.files.length !== 20) {
    ...
    database.close();
    await store.close?.();
    if (applied.length !== serverMigrationManifest.sqlite.files.length || applied.at(-1)?.name !== "20260908180502_schema_organization") {

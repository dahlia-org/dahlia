...
CREATE POLICY "account_settings_owner" ON "app"."account_settings" AS PERMISSIVE FOR ALL TO public USING ("app"."account_settings"."user_id" = nullif(current_setting('app.user_id', true), '')) WITH CHECK ("app"."account_settings"."user_id" = nullif(current_setting('app.user_id', true), ''));--> statement-breakpoint
...
CREATE POLICY "summary_job_owner" ON "app"."jobs_summary" AS PERMISSIVE FOR ALL TO public USING ("app"."jobs_summary"."owner_user_id" = nullif(current_setting('app.user_id', true), '')) WITH CHECK ("app"."jobs_summary"."owner_user_id" = nullif(current_setting('app.user_id', true), ''));--> statement-breakpoint

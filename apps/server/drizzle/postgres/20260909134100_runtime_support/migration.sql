...
CREATE POLICY "search_document_select" ON "search"."documents" AS PERMISSIVE FOR SELECT TO public USING ("app"."current_identity_can_read_vault"("search"."documents"."vault_id"));--> statement-breakpoint
...
CREATE POLICY "vault_key_read" ON "crypto"."vault_keys" AS PERMISSIVE FOR SELECT TO public USING ("crypto"."vault_keys"."owner_user_id" = nullif(current_setting('app.user_id', true), '')::uuid OR "app"."current_identity_can_read_vault"("crypto"."vault_keys"."vault_id"));--> statement-breakpoint
CREATE POLICY "vault_key_write" ON "crypto"."vault_keys" AS PERMISSIVE FOR ALL TO public USING ("crypto"."vault_keys"."owner_user_id" = nullif(current_setting('app.user_id', true), '')::uuid) WITH CHECK ("crypto"."vault_keys"."owner_user_id" = nullif(current_setting('app.user_id', true), '')::uuid);--> statement-breakpoint

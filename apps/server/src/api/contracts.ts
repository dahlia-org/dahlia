...
export const contracts: Record<OperationId, RouteConfig & { operationId: string }> = {
  ...
  listServerOrganizations: route("get", "/api/v1/admin/organizations", "listServerOrganizations", "Administrator Team Organization directory", { 200: json(z.object({ items: z.array(S.organization.extend({ memberCount: S.integer, teamCount: S.integer })), hasMore: z.boolean() })) }, { query: z.object({ offset: z.string().regex(/^\d+$/).optional() }).strict() }, browser),
  getServerOrganization: route("get", "/api/v1/admin/organizations/{organizationId}", "getServerOrganization", "Team Organization directory details; administrator only, independent of membership", { 200: json(S.organization.extend({

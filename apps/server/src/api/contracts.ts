...
export const contracts: Record<OperationId, RouteConfig & { operationId: string }> = {
  ...
  getRelocations: route("get", `${v}/relocations`, "getRelocations", "Resolve moved resources to currently accessible Workspaces", { 200: json(z.object({ workspaces: z.array(S.workspaceRead), items: z.array(z.object({ entity: z.enum(["project", "meeting", "file"]), id: S.id, workspaceId: S.id })) })) }),
  ...
  forceDeleteWorkspace: route("delete", `${o}/workspaces/{workspaceId}`, "forceDeleteWorkspace", "Delete an Organization Workspace after confirmation", { 200: json(S.receipt) }, body(z.object({ id: S.id, revision: S.integer, changeCursor: S.cursor }).strict()), browser),
  ...
  deleteOrganization: route("delete", "/api/v1/admin/organizations/{organizationId}", "deleteOrganization", "Delete an empty Organization; Server administrator only", { 204: empty }),
  createOrganization: route("post", "/api/v1/organizations", "createOrganization", "Create an Organization with an explicit initial owner; Server administrator only", { 201: json(S.organization) }, body(createOrganizationSchema)),

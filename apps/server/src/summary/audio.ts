...
export function createAudioSummaryMethod(config: AppConfig, store: MeetingSyncStore, sync: MeetingSyncService,
  ...
  return {
    ...
    captureSettings: (settings, detail) => ({
      model: settings.processing.remote.summaryModel ?? (audioProvider.backend === "databricks" ? "system.ai.gemini-3-8-flash" : "gemini-3-8-flash"), reasoningEffort: settings.processing.remote.reasoningEffort ?? "medium",

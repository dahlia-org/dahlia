...
const checkpointSchema = z.object({ vaultId: z.uuid(), meetingId: z.uuid(), generation: z.string(),
...
export async function transcriptCheckpoint<T>(vaultId: string, meetingId: string, generation: string,
  ...
  if (after !== undefined) {
    ...
    if (previous.vaultId !== vaultId || previous.meetingId !== meetingId) throw new RequestError(400, "invalid_transcript_after");
  ...
  return { items: records.slice(start, end), hasMore: end < records.length,
    next_after: btoa(JSON.stringify({ vaultId, meetingId, generation, position: end, digest: await digest(end) })) };
}

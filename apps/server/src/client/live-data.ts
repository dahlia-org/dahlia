...
export function useLiveQuery<T>(key: string | undefined, load: (signal: AbortSignal, previous?: T) => Promise<T>) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(liveDataEvent, refresh);
    ...
    return () => {
      current.dispose();
      queue.current = null;
      window.removeEventListener(liveDataEvent, refresh);
      window.removeEventListener(clientMutationEvent, refresh);
...
export function useLiveJSON<T>(url?: string) {
  return useLiveQuery<T>(url, (signal) => json<T>(url!, { signal }));
}

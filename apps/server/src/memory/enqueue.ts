...
export async function enqueueMemorySource(db: NodePgDatabase, schema: typeof Schema, workspaceId: string,
  kind: "meeting" | "shared", sourceId: string, reconcile = false) {
  const state = schema.workspaceMemoryState;
  const [workspace] = await db.select({ purge: state.purge }).from(state).where(eq(state.workspaceId, workspaceId));
  if (!workspace || workspace.purge) return;
  const jobs = schema.memorySourceJob;
  const values = { workspaceId, kind, sourceId, documentId: `${kind}-${sourceId}` };
  const insert = db.insert(jobs).values(values);
  const rows = await (reconcile ? insert.onConflictDoNothing() : insert.onConflictDoUpdate({
    target: [jobs.workspaceId, jobs.documentId], set: { generation: sql`${jobs.generation} + 1` },
  })).returning();
  if (rows.length) await db.update(state).set({ generation: sql`${state.generation} + 1`, status: "pending",
    availableAt: new Date(), attempts: 0, errorCode: null }).where(and(eq(state.workspaceId, workspaceId), eq(state.purge, false)));
}

...
へ保存しない。Databricks backend の管理画面は forwarded user token と catalog read scopes で Unity Catalog Model Services API の `system.ai` 一覧を全ページ都度取得し、一覧自体は保存せず、管理者が有効化した Model Alias だけを application database に保存する。

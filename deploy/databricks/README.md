...
The default App names are `dahlia-dev` for `dev` and `dahlia-prod` for `prod`. The corresponding Lakebase project IDs are `dahlia-db-dev` and `dahlia-db`. Development binds artifact storage to the existing `dahlia_prod_catalog.default.dahlia_artifacts_dev` Volume. Production provisions `main.dahlia.artifacts`, but the App remains bound to `main.default.dahlia_artifacts` until its bytes are copied and verified; switch the `dahlia_artifacts` resource and App binding only as part of that explicit migration.

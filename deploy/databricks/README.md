...
Configure `DAHLIA_AUTH_SECRET` for stable Better Auth browser sessions in this deployment example. The environment variable is recommended; without it, Dahlia reads or creates `dahlia-auth-secret` in its working directory, whose persistence is the operator's responsibility. Before deploying this bundle, create a unique random secret of at least 32 characters in an existing Databricks secret scope. The bundle reads scope `dahlia`, key `better-auth-{target}` through an App Secret resource with `READ` permission; override `better_auth_secret_scope` and `better_auth_secret_key` to use other names. Never put the value in bundle variables or source files. Header authentication uses `DAHLIA_AUTH_HEADER` (default `X-Forwarded-Email`) as the email identity and stores its normalized value in `account.account_id`. New users join their email-domain Organization; the first is owner and later users are members. Departed or removed users are not automatically added again.

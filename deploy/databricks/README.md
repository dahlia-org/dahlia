...
The default App names are `dahlia-dev` for `dev` and `dahlia-prod` for `prod`. The corresponding Lakebase project IDs are `dahlia-db-dev` and `dahlia-db`. Both targets use their configured catalog with the default `server` schema and managed `storage` Volume.
...
Use `-t prod` for production. Development uses separate disposable resources.

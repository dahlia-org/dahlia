...
- rustup の Rust 1.95.0（署名付きアプリのビルド用）

Dahlia は、同梱 Codex の状態と認証を他の Codex アプリや Codex CLI から分離して管理します。要約を生成する前に、**設定 → AI 接続**から明示的にサインインしてください。ブラウザを使った ChatGPT 認証は Dahlia の Application Support ディレクトリに保存されます。

従来の OpenAI/Databricks 向け最大出力トークン設定は、固定した Codex app-server プロトコルに同等のターン単位パラメータがないため、Codex 要約には適用されません。既存のプロバイダー設定と Keychain 値は削除せず、未使用のまま保持します。
...
> **注意:** `swift run` には同梱 Codex ヘルパーがなく、Data Protection Keychain も使用できません。フル機能には `run-dev.sh` を使用してください。アプリバンドル用スクリプトの初回実行時は、固定した Codex ソースを取得し、Codex 0.144.4 を Cargo で `aarch64-apple-darwin` 向けにビルドします。

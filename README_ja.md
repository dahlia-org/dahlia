...
Databricks AI Gateway の認証には Databricks CLI 経由の OAuth U2M を使用します。モデル設定からサインインする前に、現行版の CLI をインストールしてください。Dahlia は `databricks auth token` で必要時に短期トークンを取得し、Databricks トークン自体は保存しません。
